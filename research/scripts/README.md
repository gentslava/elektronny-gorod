...
│  ────────────────────                                          │
│  check-app-version.sh      ← сравнить APP_VERSION в const.py   │
│                              с fresh original APK/APKS          │
│                              exit 0 = up-to-date, 1 = устарел  │
│  07-auth-flow.sh           ← ввод login/password/phone из      │
│                              gitignored auth.env; SMS manual    │
└────────────────────────────────────────────────────────────────┘
```
...
Автоввод в уже открытый AVD 1080×2400:

```bash
./research/scripts/07-auth-flow.sh password
./research/scripts/07-auth-flow.sh sms
```

SMS-код вводится напрямую в AVD и не сохраняется.
