...
- Полный PCAP штатного приложения 2026-07-13: на каждый ring оно делает
  `REGISTER → INVITE → 100 Trying` и держит INVITE до ответа/`CANCEL` (~24с).
  Значит короткий pre-answer hold является штатным, а не HA-специфичным действием.
- Для одной связанной пары панелей звонок с A переводит B в «Занято» даже при
  полностью отключённых FCM/SIP механизмах HA и только со штатным приложением.
  Панели имеют разные access-control id, но общий place. PCAP не позволяет
  определить внутреннее правило оператора, поэтому причина формулируется только
  как группировка на стороне оператора/оборудования, без более узкой гипотезы.
- Постоянную или авто-отвечающую регистрацию всё равно не держим: регистрация
  транзиентна и живёт только в окне реального вызова, как в приложении.
...
- На home.server всё равно `downlink=0` — **расхождение пробы с интеграцией** (сравнение
  ...
  тогдашней модели интеграции (локальный SDP + latching, uplink после 200 OK,
  register-on-answer)
