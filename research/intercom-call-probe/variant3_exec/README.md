# Variant #3: exec-backchannel (механизм проверен 2026-06-24)

> **ЭКСПЕРИМЕНТ** — scaffolding для сравнения с выбранным вариантом #1 (WS-binary).
> **Обновление 2026-06-24:** на живом тестовом go2rtc — exec-source **исполняется**
> (forward-медиа ок), но `#backchannel=1` **не отдаётся pull-консьюмеру**
> (`codecs not matched` — **send-only #2084 воспроизведён**); нужен WebRTC-консьюмер с
> two-way audio. Самый сложный вариант. См. [`../FINDINGS.md`](../FINDINGS.md)
> §«D-audio-variants». **НЕ деплоить автоматически — продакшн-go2rtc не трогать.**
