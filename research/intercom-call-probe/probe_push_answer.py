...
# --- ОФФЛАЙН-OVERRIDE (только для test_harness/door_emulator.py; в обычной работе пусты) ---
SIP_SERVER = os.environ.get("SIP_SERVER") or None  # если задан — слать SIP сюда вместо realm-DNS
# фейк-креды: пропускают mint_sip (без оператора/FCM). TEST_REALM используется и как SIP-domain.
TEST_LOGIN = os.environ.get("TEST_LOGIN") or None
TEST_PASSWORD = os.environ.get("TEST_PASSWORD")
TEST_REALM = os.environ.get("TEST_REALM") or None
...
async def main() -> None:
    ...
    # ОФФЛАЙН (TEST_LOGIN+TEST_REALM): session.json/firebase_config.json/FCM не нужны —
    # эмулятор поднимает SIP/RTP сам. PERSIST_REG обязателен (FCM-flow пропущен).
    offline = bool(TEST_LOGIN and TEST_REALM)
    if offline:
        ua = common.build_user_agent()
        sess = common.Session(
            access_token="", refresh_token=None, operator_id="0", account_id="0",
            subscriber_id="0", phone="0", user_agent=ua, install_id="offline-test",
            intercoms=[{"placeId": "0", "accessControlId": "0", "name": "offline"}],
        )
        cfg = {"messaging_sender_id": "0"}
        intercom = sess.intercoms[0]
    else:
        from firebase_messaging import FcmPushClient, FcmRegisterConfig  # noqa: F401
    ...
    # ОФФЛАЙН: TEST_LOGIN/PASSWORD/REALM заданы → пропускаем mint_sip (без оператора/FCM).
    if TEST_LOGIN and TEST_REALM:
        creds = {"login": TEST_LOGIN, "password": TEST_PASSWORD or "", "realm": TEST_REALM}
        log(f"OFFLINE: фейк-креды из env (login={TEST_LOGIN}, realm={TEST_REALM}) — mint_sip пропущен.")
    else:
        creds = await mint_sip(sess, intercom)
    # ОФФЛАЙН: SIP_SERVER задан → SIP идёт на эмулятор вместо DNS-резолва realm.
    ip = SIP_SERVER if SIP_SERVER else socket.gethostbyname(creds["realm"])
    ...
    log(f"SIP-креды готовы (realm={creds['realm']}, sip_server={ip}, local_ip={local_ip}). SIP НЕ зарегистрирован.")
