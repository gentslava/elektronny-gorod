...
Локальные `.har` / `.chlz` файлы — gitignored. Их содержимое (имена, размеры, конкретные сценарии) **не документируется в публичных файлах** (включая этот README). Сводное покрытие сценариев — в [`docs/architecture/api-reference.md` §Source HAR](../../docs/architecture/api-reference.md) в форме `(дата + аккаунт-плейсхолдер + бренд + сценарий)`, без имён файлов и реальных идентификаторов.
