...
async def test_hangup_during_answer_does_not_orphan_bridge():
    # Гонка B P2-1: пока async_answer внутри _answer_lock await-ит bridge.start
    # (self._bridge ещё None), параллельный async_hangup НЕ должен забрать teardown
    # раньше времени. Лок сериализует: answer завершает setup → присваивает мост →
    # потом hangup снимает его. Итог: bridge.stop вызван, _bridge is None (нет
    # orphaned ffmpeg + HTTP-listener :40020).
    c = _go2rtc_controller()
    c.handle_signal(_ring())
    ...
    async def _blocking_start() -> None:
        started.set()  # answer вошёл в _setup_audio_bridge
        await release.wait()  # держим окно гонки открытым

    held = MagicMock()
    held.holding = True
    held.in_call = False
    held.accept = AsyncMock(return_value=True)
    held.async_hangup = AsyncMock()
    c._manager = held
    ...
    with patch(f"{_CC}.AudioBridge") as BridgeCls, patch(
        ...
        bridge = BridgeCls.return_value
        bridge.start = AsyncMock(side_effect=_blocking_start)
        bridge.stop = AsyncMock()

        answer_task = asyncio.create_task(c.async_answer())
        await started.wait()  # answer держит лок, висит на bridge.start
        hangup_task = asyncio.create_task(c.async_hangup())
        await asyncio.sleep(0)  # дать hangup попытаться войти (должен ждать лок)
        release.set()  # отпускаем bridge.start → answer достроит мост
        answered, _ = await asyncio.gather(answer_task, hangup_task)

    assert answered is True
    bridge.stop.assert_awaited_once()  # мост снят hangup'ом (не утёк)
    assert c._bridge is None  # нет orphaned ffmpeg/порта
...
async def test_uplink_sink_cleared_on_release_held():
    # _async_release_held (FCM ended при держимом) тоже снимает sink (не течёт).
    c = _go2rtc_controller()
    held = MagicMock()
    held.holding = True
    held.async_hangup = AsyncMock()
    c._manager = held
    c._uplink_sink = MagicMock()
    await c._async_release_held()
    assert c._uplink_sink is None
    held.async_hangup.assert_awaited_once()
    assert c._manager is None
