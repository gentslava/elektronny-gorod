...
Ваши **домофоны, камеры и замки** Электронного города (Новотелеком) и Дом.ру — прямо в Home Assistant. Гость позвонил в дверь — вы мгновенно видите видео, слышите звук, отвечаете голосом и открываете дверь одним касанием, даже если вы не дома.

Интеграция воспроизводит API официальных приложений «Мой Дом» и «Умный Дом.ру»: смотрите камеры со звуком, принимайте и завершайте вызовы, просматривайте историю принятых и пропущенных звонков и стройте на этом любые автоматизации Home Assistant.

> 🛠️ **4.0.1 — рекомендуемое обновление для пользователей 4.0.0:** звонки
> снова доходят через FCM, ошибочное соединение больше не может подвесить Home
> Assistant или бесконечно раздувать журнал. Установите 4.0.1 через HACS, когда обновление будет предложено, и
> перезапустите Home Assistant; повторная авторизация не нужна. Подробности — в
...
- [Что нового в 4.1.0](#что-нового-в-410)
- [Что было в 4.0.1](#что-было-в-401)
- [Главное в ветке 4.0](#главное-в-ветке-40)
...
- [🎞 Архив камер в медиабраузере](#-архив-камер-в-медиабраузере)
...
- **Архив камер в медиабраузере:** в разделе «Мультимедиа» появился источник «Электронный город» — аккаунт → адрес → камера → день → события движения.
- **Просмотр записи прямо в браузере:** клип открывается по клику, перемотка работает.
- **Глубина по сроку хранения оператора:** 14 дней для домофонов, 7 дней для остальных камер. Запрос за пределами окна даёт понятное сообщение, а не ошибку камеры.
- **Скрытые камеры остаются скрытыми:** камеры, выключенные переключателем «Показывать на панели», не попадают в архив ни в списке, ни при воспроизведении.
- **Ссылка оператора не покидает сервер:** Home Assistant забирает файл сам и отдаёт его по собственной короткоживущей ссылке; подписанный URL оператора не попадает ни в браузер, ни в журнал.
- **Обновление без перенастройки:** установите 4.1.0 через HACS и перезапустите Home Assistant. Переавторизация не нужна.

Обновление не содержит breaking changes. Подробности, ограничения среза и порядок обновления — в [`docs/releases/4.1.0.md`](docs/releases/4.1.0.md). Изменения предыдущего выпуска описаны в [`docs/releases/4.0.1.md`](docs/releases/4.0.1.md).

## Что было в 4.0.1

- **Звонки снова доходят:** интеграция разбирает новый VAPID-формат служебных FCM-заголовков, из-за которого событие вызова терялось до отправки в Home Assistant.
- **Home Assistant не зависает:** возвращён конечный предохранитель для повреждённого FCM-соединения; горячий цикл с постоянно растущим traceback больше не блокирует event loop.
- **Журнал не разрастается бесконечно:** после одной контрольной попытки circuit breaker ограничивает переподключения проблемного аккаунта и показывает понятное предупреждение в Repairs.
- **Остальные функции продолжают работать:** камеры, замки, баланс, история и другие аккаунты не останавливаются из-за сбоя FCM одной config entry.
- **Обновление без перенастройки:** установите 4.0.1 через HACS и перезапустите Home Assistant. Удалять интеграцию или проходить авторизацию заново не нужно.

Обновление не содержит breaking changes. Причина, проверка на реальном звонке и пошаговое обновление описаны в [`docs/releases/4.0.1.md`](docs/releases/4.0.1.md).

## Главное в ветке 4.0

- **Вызов без polling:** входящий звонок приходит через FCM и сразу доступен автоматизациям Home Assistant.
- **Ответ и разговор:** SIP-приём, завершение вызова, видео и звук гостя, микрофон браузера и сервисы `elektronny_gorod.answer` / `hangup`.
- **Готовый экран вызова:** `custom:eg-intercom-call-card` объединяет видео, ответ, микрофон, звук и безопасное открытие двери; поддерживает телефон, desktop и настенную панель.
- **История событий:** `custom:eg-event-history-card` показывает принятые и пропущенные вызовы, группирует их по датам и устройствам и объединяет несколько мест или аккаунтов.
- **События для автоматизаций:** новые принятые и пропущенные вызовы появляются как HA `event` entities. История движения камеры доступна отдельной сущностью, отключённой по умолчанию; её включение запускает polling выбранной камеры.
- **Стабильный внешний RTSP:** включённые камеры можно отдельно опубликовать через go2rtc для NVR, медиаплеера или другой локальной системы. Публикация выключена по умолчанию; скрытые камеры подключаются отдельной настройкой.
- **Обновление без перенастройки:** повторно добавлять интеграцию в Home Assistant не требуется.
- **Надёжность:** исправлены FCM reconnect/watchdog, смена звонящего, конкурентное открытие call-video и lifecycle SIP/go2rtc.

Большой обзор возможностей, появившихся в 4.0.0, ограничения и первоначальная настройка собраны в [`docs/releases/4.0.0.md`](docs/releases/4.0.0.md).
...
- **История принятых и пропущенных вызовов** — отдельные сущности по местам и общая Lovelace-карточка с фильтрами и постраничной загрузкой.
- **Архив записей камер в медиабраузере** — просмотр событий движения по дням и воспроизведение клипов с перемоткой (см. [раздел ниже](#-архив-камер-в-медиабраузере)).
...
Создаваемые типы сущностей: `camera` (обычное и активное call-video), `lock` (открытие двери), `event` (вызовы и история), `sensor` (баланс, дни до блокировки, состояние вызова), `binary_sensor` и `switch`.
...
- Опционально публиковать готовые потоки по стабильным RTSP-адресам для NVR, медиаплееров и других локальных клиентов.

### Как подключить

Интеграции нужен доступный экземпляр go2rtc с HTTP API и RTSP-портом. Добавлять камеры из интеграции в `go2rtc.yaml` вручную не требуется: потоки создаются и обновляются автоматически.

#### Если go2rtc уже установлен

Можно использовать существующий go2rtc — например, отдельный add-on, контейнер или сервер в локальной сети:

1. Убедитесь, что Home Assistant видит HTTP API go2rtc (обычно `http://HOST:1984`) и RTSP-порт `8554`.
2. При добавлении интеграции выберите **«Настроить go2rtc»** и укажите URL API.
3. Если на go2rtc настроена Basic Auth, укажите имя пользователя и пароль в той же форме.

Для отдельного контейнера или сервера используйте его имя хоста либо локальный IP-адрес, доступный из Home Assistant. Адрес `127.0.0.1` подходит только тогда, когда go2rtc запущен в том же окружении, что и Home Assistant Core.

#### Если go2rtc ещё нет: WebRTC Camera через HACS

[WebRTC Camera](https://github.com/AlexxIT/WebRTC) автоматически загружает и запускает собственный go2rtc, поэтому отдельный add-on или ручная установка бинарного файла не нужны:
...
4. В настройках интеграции включите go2rtc и оставьте стандартный адрес `http://127.0.0.1:1984`.

Вместе с компонентом станет доступна Lovelace-карточка с низкой задержкой и звуком:
...
Замените `camera.YOUR_INTERCOM` на идентификатор нужной камеры из Home Assistant. При управлении ресурсами Lovelace через YAML выполните [дополнительный шаг из документации WebRTC Camera](https://github.com/AlexxIT/WebRTC#installation).

#### Если интеграция уже настроена

Откройте её параметры и включите go2rtc — повторно добавлять учётную запись и устройства не требуется.

**Примечание:** Для работы аудио и низкой задержки используйте актуальные версии go2rtc, WebRTC Camera и Home Assistant.

### Внешний RTSP в ветке 4.0

В настройках интеграции go2rtc появились две отдельные опции:

- **«Публиковать включённые камеры для внешнего RTSP»** — создаёт стабильные потоки только для включённых в Home Assistant сущностей камер.
- **«Также публиковать скрытые камеры»** — дополнительно включает скрытые сущности и работает только вместе с основной опцией. Отключённые сущности камер не публикуются никогда.

Обе опции по умолчанию выключены. Для опубликованных камер интеграция сразу запускает фоновый go2rtc consumer: он не даёт одноразовой операторской ссылке истечь до первого просмотра. Источник обновляется каждые 28 минут 30 секунд без сброса уже подключённого зрителя. Готовые адреса и фактическое состояние потоков отображаются диагностической сущностью **«Опубликованные RTSP-потоки»**; учётные данные go2rtc в её атрибуты не попадают.

Настройка публикации влияет только на фоновый keep-warm. Явное открытие включённой скрытой камеры в Home Assistant продолжает работать без постоянного preload. Интеграция не открывает RTSP-порт в Интернет — сетевой доступ, firewall, VPN и безопасность go2rtc остаются ответственностью пользователя.
...
Просматривайте принятые и пропущенные вызовы в отдельной карточке Home Assistant и используйте новые вызовы в автоматизациях.
...
Карточка истории входит в тот же файл, что и экран вызова. Добавьте ресурс один раз:
...
Для нескольких мест или настроенных аккаунтов используйте `entities:` — карточка объединит ленты по времени и сохранит раздельные фильтры устройств. Тексты оператора и персональные данные в карточке не показываются.

Подробная настройка и ограничения: [`history-card.md`](docs/features/mobile-app-parity/history-card.md).

## 🎞 Архив камер в медиабраузере

Записи событий движения доступны в **Мультимедиа → Электронный город**. Дерево повторяет логику приложения: аккаунт → адрес → камера → день → события. Каждое событие подписано временем и длительностью записи; недоступные для просмотра помечены и не открываются.

Глубина списка соответствует сроку хранения у оператора: **14 дней** для домофонов и **7 дней** для остальных камер. Запрос за пределами этого окна даёт понятное сообщение, а не общую ошибку камеры.

Клип открывается по клику прямо в браузере, перемотка работает. Оператор готовит mp4 по требованию, поэтому первый клик по свежему событию занимает несколько секунд — интеграция дожидается готовности сама.

**Что важно знать про доступ.** Ссылка на запись, которую получает браузер, ведёт на сам Home Assistant и действует ограниченное время. Открывается она без повторного входа — так же устроены штатные прокси камер и TTS в Home Assistant, иначе тег `<video>` не смог бы забрать файл. Пересылать такую ссылку посторонним не стоит. Подписанная ссылка оператора при этом не покидает сервер и не попадает в журнал.

Камеры, скрытые переключателем «Показывать на панели», в архив не попадают — ни в списке, ни при попытке воспроизведения.

**Ограничения текущего среза:** доступны только клипы по событиям движения (перемотки по непрерывному таймлайну нет), эскизы событий не передаются, личные камеры не поддерживаются.
