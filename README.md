...
Номер телефона или договора, привязанный к вашей учётной записи у оператора, и доступ к SMS на этот номер. Если у оператора для вашей учётной записи задан пароль, SMS не потребуется — пароль спросят вместо кода.

go2rtc не обязателен. Без него камеры работают, но без звука; подробнее — в разделе [Подключение камер через go2rtc](#подключение-камер-через-go2rtc).
...
### Что спрашивают при настройке

| Шаг | Поле | Что вводить |
|---|---|---|
| Вход | Номер телефона или договора | Номер, привязанный к учётной записи у оператора |
| Вход → Для опытных | Токен доступа | Необязательно. Если токен уже есть, вход по SMS не потребуется — поле спрятано в свёрнутой секции |
| Договор | Договор | Выбор из найденных по номеру; показываются адрес и номер счёта |
| Код из SMS | Код | Четыре цифры из сообщения оператора |
| Пароль | Пароль | Спрашивают вместо SMS, если пароль задан у оператора |
| go2rtc | — | Меню: настроить go2rtc сразу или пропустить и добавить позже |

Одну и ту же учётную запись можно добавить несколько раз — по одной записи на договор.

### Параметры go2rtc

Задаются при настройке и потом меняются в «Настроить» у записи интеграции.

| Параметр | По умолчанию | Назначение |
|---|---|---|
| Использовать go2rtc | выключено | Включает звук в потоке камер. Без него видео работает, звука нет |
| Адрес API go2rtc | `http://127.0.0.1:1984` | Куда интеграция ходит за потоками. Проверяется при сохранении |
| Логин go2rtc | пусто | Только если на go2rtc включена авторизация |
| Пароль go2rtc | пусто | Только если на go2rtc включена авторизация |
| Публиковать включённые камеры для внешнего RTSP | выключено | Держит потоки поднятыми, чтобы их можно было забрать снаружи — например во Frigate |
| Публиковать и скрытые камеры | выключено | Работает только вместе с предыдущим. Отключённые в Home Assistant сущности не публикуются никогда |

Публикация наружу включается вручную и по умолчанию выключена: постоянно поднятые потоки нагружают и оператора, и go2rtc.

## Удаление

1. Конфигурация → Устройства и службы → «Электронный город».
2. У нужной записи — меню «⋮» → «Удалить».

Что происходит при удалении:

- сущности, устройства и история их состояний удаляются вместе с записью;
- push-токен отвязывается у оператора, звонки в домофон перестают приходить на этот Home Assistant;
- потоки вида `eg_<идентификатор камеры>` **остаются в конфигурации go2rtc** — интеграция их не удаляет. Если go2rtc используется только для неё, эти строки можно убрать вручную из `go2rtc.yaml`.

Чтобы убрать саму интеграцию, а не только учётную запись: в HACS откройте «Электронный город» и выберите «Удалить», либо, при ручной установке, удалите директорию `custom_components/elektronny_gorod`. После этого перезапустите Home Assistant.
