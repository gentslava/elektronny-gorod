...
- Для нетривиального плана заранее фиксировать subagent-driven execution mode,
  implementation ownership и independent reviewer matrix. Перед обычным push/PR
  требовать clean committed candidate и candidate-bound approvals по
  ADR-0015. После approvals передать Validator/root публикацию durable PR
  evidence comment и проверку `CI_GREEN` по ADR-0015.

## Когда привлекать другие роли

- **Security**: правки `http.py`, `config_flow.py:logging`, `helpers.py`,
  `diagnostics.py`, `fcm.py`, auth/token/credentials. Hand-off →
  `security-auditor` для precheck и independent post-freeze review.
- **HA-expert**: `manifest.json`, `config_flow.py`, entity, coordinator, Repairs,
  FCM lifecycle, IQS. Hand-off → `ha-expert`.
...
- Не закрывать `REVIEW_OK` self-review-ом и не переносить approvals старого
  base/head/tree на новый candidate.
