...
1. `docs/decisions/0006-mirror-app-behavior.md` — **главный принцип**.
2. `docs/decisions/0007-stateful-emulator-baseline.md` — pipeline дизайн.
3. `docs/architecture/api-reference.md` — текущее состояние API knowledge.
4. `docs/aidd/runbooks/har-collection.md` — workflow.
5. `research/scripts/README.md` — pipeline.
...
- **Обновление `docs/architecture/api-reference.md`** — строго на основе фактов из HAR, без гипотез.
...
- `source-driven-development` — сверка с реальностью (HAR), не с догадками.
- `context-engineering` — не загружать в контекст лишнее (`api-reference.md` + конкретный HAR, не весь репо).
- `documentation-and-adrs` — для обновления `api-reference.md` структурированно.
