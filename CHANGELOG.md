...
- **Options flow: нельзя было очистить go2rtc creds при auth на сервере** ([A-78](docs/audit/project-audit.md)). Юзер открывал «Настройки go2rtc», очищал поля username/password, save → форма показывала generic «Не удалось подключиться к go2rtc» и перерисовывалась с прежними creds (defaults перезаливались из options). Root cause: `validate_go2rtc` маппил HTTP **401** на `go2rtc_unreachable`, хотя сервер был достижим — просто отверг auth-запрос без `Authorization` header. **Fix**: различить 401 → отдельный error code `go2rtc_auth_failed` с понятным сообщением: «go2rtc требует auth. Введите creds, отключите auth на сервере, или снимите галочку Use go2rtc чтобы сохранить без подключения». Workaround до релиза: снять галочку «Использовать go2rtc» — валидация скипается.
