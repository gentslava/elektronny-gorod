...
- **Camera: revert update_source() + cache invalidate on PUT failure** ([A-70](docs/audit/project-audit.md), partial revert [A-66](docs/audit/project-audit.md)). Production-лог 2026-05-27 показал continuous «мигание видео» даже после deploy [A-68](docs/audit/project-audit.md) (concurrent dedup) + [A-69](docs/audit/project-audit.md) (TTL cache 30s): operator выдаёт unique token на каждый запрос → cache miss = inevitable PUT с новым src → `Stream.update_source()` restart → 1-2 sec interruption видео каждые 30-60 сек. Restart сам себя провоцировал (прерывание active ffmpeg consumer → token expire → next fetch unique URL → restart). **Fix**: убран `stream.update_source()` после PUT (A-66 partial revert). Trade-off: lag 10-30s после реального token expire (HA Stream lifecycle сам восстанавливается через retry-backoff → fresh stream_source → fresh PUT → reconnect) **vs** continuous flicker каждые 30-60s. Lag реже + acceptable. Дополнительно A-70: `try/except` вокруг `_go2rtc_upsert_stream` PUT — при failure invalidate `_cached_stream_url` для faster recovery (next call не ждёт 30s TTL expire). 3 unit-теста (`tests/test_camera_a70_invalidate.py`): regression-guard для revert + invalidate-on-failure + cache populate.

- **Camera: TTL cache stream URL** ([A-69](docs/audit/project-audit.md)). Дополняет [A-68](docs/audit/project-audit.md) — A-68 future-pattern дедупит **одновременные** calls (overlap во времени), но не покрывает **sequential batches** с интервалом 0.5-30 сек. Production-лог 2026-05-27 13:31:02→13:31:03.7 показал 6 cameras в batch #1, через 0.9 сек batch #2 для тех же cameras → 6× `forced HA Stream restart` → видео мигает. **Fix**: 30s TTL cache в `_fetch_stream_source_impl`. Cache hit → return cached URL → `_ensure_go2rtc_stream` видит `src == _last_src` → skip PUT → НЕТ `Stream.update_source()` restart. Cache TTL < operator session token TTL (~минуты) → URL всегда живой. Failure (empty URL) НЕ кэшируется → next call fresh. **Subsumes A-67** (cold-start warmup): первый legitimate call от HA Stream preload делает fresh PUT, последующие retry batches в TTL → cache hit, no extra HTTP при startup. 4 unit-теста (`tests/test_camera_stream_ttl_cache.py`).
