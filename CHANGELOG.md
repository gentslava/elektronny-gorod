...
- **Camera: go2rtc cache invalidation на hidden→visible transition** ([A-66](docs/audit/project-audit.md)). Operator FLV URL содержит session token с коротким TTL (минуты). Пока камера была hidden (см. [A-63](docs/audit/project-audit.md)), кешированный в go2rtc ffmpeg producer URL устаревал. При un-hide HA Stream worker подключался к go2rtc → ffmpeg пытался открыть upstream с просроченным URL → `Invalid data found` → HA retry с backoff (10s → 30s → 1m). На retry оптимизация `_last_src == src` (`camera.py:_ensure_go2rtc_stream`) могла skip-нуть PUT даже если operator вернул тот же URL → producer оставался с тухлым src → UX «когда вклю[ча]ю видимость, она не прогружается». Fix: tracking `_was_hidden` флага в `stream_source()` и `async_camera_image()` — при skip из-за `_is_hidden()=True` помечаем; на следующем visible вызове stream_source перед `_ensure_go2rtc_stream` сбрасываем `self._last_src = None` → forces fresh PUT с свежим operator URL даже при идентичном src. Regression-guard: повторные visible→visible вызовы продолжают skip PUT (оптимизация сохранена). 4 новых теста (`tests/test_camera_go2rtc_invalidate.py`). Эффект: видео грузится сразу после toggle «Показывать на панели», без 10-30 сек retry-окна.
