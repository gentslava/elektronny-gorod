...
- **Приём вызова домофона по SIP (фундамент two-way audio)** ([A-81](docs/audit/project-audit.md), [call-answer-model](docs/features/intercom-two-way-audio/call-answer-model.md)). Новый SIP-стек (`custom_components/elektronny_gorod/sip/` — 11 модулей) + HA-интеграция: сервисы `elektronny_gorod.answer` / `elektronny_gorod.hangup` позволяют **принять активный вызов домофона и поднять uplink-аудио к панели**. Модель — **REGISTER-on-answer** (доказано pcap реального приложения + 6 live-проб, см. call-answer-model.md): интеграция **не** держит постоянную SIP-регистрацию, а по явному «ответить» (в окне `CallInvalidated` ~30с) минтит SIP-креды (`api.mint_sip_device` → `POST .../sipdevices` с `installationId`), шлёт `REGISTER` (Expires=30, проприетарные push-params), принимает форкнутый сервером `INVITE`, отвечает `200 OK` **немедленно** (SDP: локальный адрес, G.711 PCMU/PCMA, без STUN/ICE/SRTP), сразу шлёт RTP-uplink + STUN-keepalive → активирует FreeSWITCH RTP-latching. Ответ строго привязан к `Call-ID` из FCM `CALL_INCOMING` (защита от рассинхрона — не отвечать на сброшенный вызов). SIP-стек: `audio` (G.711↔PCM), `stun` (STUN parse), `digest` (Digest MD5 RFC2617), `sdp` (parse offer + build G.711 answer), `message` (SIP parse с multi-Via/Record-Route), `dialog` (DialogState + 200 OK + BYE), `register` (REGISTER + push-params), `rtp` (RTP G.711 latching), `protocol` (asyncio SIP-транспорт), `manager` (`SipManager` фасад), `call_controller` (HA-glue: трекинг FCM-вызова + answer/hangup). SIP `realm` (`{ac_id}.intercom.{operator}.ru`) добавлен в `SENSITIVE_KEYS` (содержит acId, парный к SIP-паролю). Тесты — `test_sip_*.py` (audio/stun/digest/sdp/message/dialog/register/rtp) + `test_api_sip.py` + `test_sip_call_controller.py` (suite 234 passed). **Scope этого слайса = приём вызова + RTP-latching (проверяемо логом, uplink к панели работает).** Downlink — вывод звука гостя в HA — следующий слайс. **Ограничение слайса**: один одновременный разговор (фикс-порты SIP/RTP, модель first-answer-wins). Mint латентно-критичен — точечный `asyncio.timeout(8с)` в `call_controller`; глобальный `ClientTimeout` остаётся открытым в [A-21](docs/audit/project-audit.md). Для UX «ответить из уведомления» — готовый blueprint `blueprints/automation/elektronny_gorod/doorbell_two_way_answer.yaml`: при вызове шлёт actionable-push (Companion App) с кнопками «Ответить» (→ `answer`) / «Сбросить» (→ `hangup`), снимает уведомление при ответе/завершении.
