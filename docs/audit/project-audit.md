...
- **Tests:** `tests/test_camera_auto_recovery.py` — 11 тестов: кэш, немедленная отдача устаревшего, потолок устаревания, тело ошибки оператора вместо кадра, отказ в фоне, первый кадр без фонового дубля, одновременное первое открытие, дедупликация, ключ по нормализованному размеру, скрытая и недоступная камера при прогретом кэше. Плюс `tests/test_call_camera.py` — экран вызова ходит мимо кэша. Фоновое обновление ожидается напрямую через задачу: `async_block_till_done()` фоновые задачи записи не ждёт, и зелёный прогон держался бы лишь на том, что мок резолвится без приостановки.
