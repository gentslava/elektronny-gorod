...
- **Окружение (не дефект ветки):** репозиторный `.venv` содержит HA `2026.2.3` и Python 3.13 при объявленном минимуме `2026.8.1` и Python 3.14 — команда из [`testing/strategy.md`](../testing/strategy.md) на нём даёт 128 падений на любом коммите, включая base. Матрица прогоняется в отдельно собранных окружениях под оба пина CI.
...
- **Status:** 🟢 **resolved-in-branch** (pending merge `fix/retry-backoff-overflow`).
...
- **Status:** 🟢 **resolved-in-branch** (pending merge `fix/website-version-from-manifest-v2`).
