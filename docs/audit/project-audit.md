...
- **Fix:** stale-while-revalidate. Свежий кадр (моложе `SNAPSHOT_FRESH_SECONDS`) отдаётся из памяти; устаревший тоже отдаётся немедленно, а обновление уходит в фоновую задачу записи. Оператора ждём только когда показать нечего — на первом открытии камеры. Отказ в фоне оставляет прошлый кадр. Фоновые обновления дедуплицируются, чтобы серия запросов не порождала серию задач.
- **Tests:** `tests/test_camera_auto_recovery.py` — 5 тестов (кэш, немедленная отдача устаревшего с фоновым обновлением, отказ в фоне, первый кадр, дедупликация). Проверено мутациями: снятие кэша, ожидание оператора вместо отдачи устаревшего и снятие дедупликации — каждая роняет прогон.
