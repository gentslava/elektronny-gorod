...
- **Известные хвосты, вынесенные за кандидат.** История событий (`history.py`) свои отказы пишет на `debug` и наружу их не поднимает: с возвратом транспорта к `debug` устойчивый отказ операторских event-эндпоинтов следа выше `debug` не оставляет. Достижимость сервиса при этом закрыта фронтом «Список адресов», а история вспомогательная, поэтому правило Silver это не рушит — но пункт заведён осознанно, а не пропущен. Там же: восстановление видимости на старте (`_sync_visibility`) работает по снимку первого обновления, и если настройки экранов в этот момент недоступны, скрытые интеграцией сущности показываются снова; поведение то же, что и до ветки.
- **Осознанная плата за разведённые фронты:** вложенность допускает пару строк «Камеры места: нет данных» и «Камеры: данные снова приходят» в одном цикле — разные фронты про разные факты. Слить их обратно значило бы вернуть то, ради чего они и разведены: отказ по общедомовым гасил бы камеру домофона у двери.
