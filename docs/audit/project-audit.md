...
- **Третий круг ревью (2026-09-05):** security, HA и code-review закрыли candidate, QA вернул одну Important. Экран вызова при отказе оператора подставлял кадр из кэша до пяти минут давности, хотя докстринг, комментарий к константе и CHANGELOG обещали «всегда живой»; обе ветки поведения при этом были зелёными, то есть контракт не проверялся ничем. Разбор показал, что фолбэк внутри фетча перепроверял ровно то, что вызывающий с карточки уже нашёл пустым (до запроса доходят, только исчерпав и точный размер, и подстановку), — то есть для дашборда он был мёртв, а единственным живым потребителем оказался путь, которому он противопоказан. Фолбэк удалён целиком; вместе с ним ушли поле `_image`, ставшее write-only, и выбор более старого кадра при наличии более свежего годного. Дополнительно: интервал MJPEG совпадал с окном свежести, а ядро отмеряет период до запроса — обновление срабатывало через тик, и половина показанных кадров была минутной давности при неизменной нагрузке; интервал взят в треть окна. Отказ снимка перестал быть виден на дефолтном уровне логирования — возвращена одна строка по фронту отказа и одна по восстановлению.
