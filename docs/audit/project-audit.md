...
- **Понижение оказалось слишком широким, и это тоже нашло ревью:** методы вроде `query_cameras` отказ глотают и возвращают пустой список, так что молчащий транспорт оставлял исчезнувшие камеры вообще без следа в журнале — на любом уровне. Правило требует одной строки на пропажу, а не нуля. Итог: строка отказа пишется по фронту — `warning` на первый отказ пары «endpoint + статус», `debug` на повторы, отметка снимается успешным ответом. Это закрывает и исходную жалобу на 288 записей в сутки, и потерю сигнала.
