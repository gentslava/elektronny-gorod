...
- **Root cause:** каждый переключатель строит полный payload из трёх пунктов, читая `coordinator.data`. После успешного POST вызывался `async_request_refresh`, который отложен дебаунсером и снимок не трогает, а `async_set_dnd` не трогала его вовсе. Сериализация действий (`PARALLEL_UPDATES = 1`) этого не решает: расходятся данные, а не порядок — что и было ошибочно заявлено в комментарии при первой правке.
- **Как закрыто:** `async_set_dnd` записывает принятый payload в снимок координатора. Дальше действует прежний отложенный refresh — он лишь подтверждает то, что оператор уже принял.
