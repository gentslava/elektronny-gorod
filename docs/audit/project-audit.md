...
- **Окружение (не дефект ветки):** репозиторный `.venv` содержит HA `2026.2.3` и Python 3.13 при объявленном минимуме `2026.8.1` и Python 3.14 — команда из [`testing/strategy.md`](../testing/strategy.md) на нём падает на любом коммите — 128 тестов на head ветки, 98 на base `0673aed`. Матрица прогоняется в отдельно собранных окружениях под оба пина CI.
