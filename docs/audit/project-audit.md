...
- **Status:** 🟡 **WON'T FIX** (решение владельца, 2026-09-05) — техническая часть закрыта, концептуальная отклонена.
- **Area:** Correctness
- **Evidence:** [`lock.py`](../../custom_components/elektronny_gorod/lock.py) — `LOCK_UNLOCK_DELAY = 5`, цикл состояния через `async_call_later`.
- **Что было исправлено:** `asyncio.sleep` в `async_update` заменён на `async_call_later` (A-46) — блокировки event loop нет.
- **Почему остальное не чиним:** исходная формулировка «домофон не закрывается физически, синтетическое состояние вводит в заблуждение» неверна. Электромагнитный замок домофона именно так и работает: открывается на несколько секунд и закрывается сам. Модель `lock` с автовозвратом отражает этот цикл точно, а `button` его потерял бы — у кнопки нет состояния, и пользователь не увидел бы, что дверь сейчас открыта. Замена `lock` → `button` сломала бы автоматизации пользователей ради ухудшения модели.
- **Follow-up:** если у какого-то оператора замок ведёт себя иначе (остаётся открытым до повторной команды), это отдельный случай — тогда нужен не переход на `button`, а корректное отражение реального состояния.
...
- **Решение = design tradeoff (ADR-0009).** Конфликт с принципом [mirror-app-behavior]: интеграция воспроизводит приложение, а приложение **намеренно** даёт зависнуть. Рассмотренные варианты:
...
- **Field evidence 2026-08-10 (первичный симптом):** пользовательский production-report показал, что `firebase-messaging` завершает `FcmPushClient` на отдельном encrypted push с `binascii.Error: Incorrect padding`. Первоначальная гипотеза про отсутствующий Base64URL padding была неполной и заменена DIAG-evidence 2026-08-13 ниже; upstream PR #37 сам по себе production-формат оператора не закрывает.
...
- **Status:** ✅ **RESOLVED** — исходная молчаливая смерть исправлена через **PR #66**; ограничение watchdog-цикла, конечный dependency fuse и нормализация Web Push headers по production evidence влиты через **PR #78** (merge `8b53fc4`).
...
- **Controlled reproduce (2026-08-13, прод):** `mtalk.google.com` намеренно заблокирован пользовательским правилом на роутере (AdGuard), чтобы проверить механизм на живой системе. Полученная последовательность точно соответствует проекту:
  ...
  --- блокировка снята ---
  09:05:40  "FCM: выполняю пробную попытку восстановления"                ← первый тик после дедлайна 09:04:40
  09:05:41  FCM doorbell listener запущен                                 ← VERIFYING
...
> `camera.py` — HA triggers. Поэтому A-82 resolved в master через PR #71; A-83
...
- **Status:** 🟡 REMEDIATION-IN-REVIEW — фикс в `master`, acceptance ждёт первого fork-PR.
...
- **Fix:** сборка zip отделена от публикации. Публикация выполняется на событии `workflow_run` в контексте base-репозитория, где токен может иметь `contents: write`. Номер PR не берётся из артефакта (он подконтролен автору fork-PR и позволял бы перезаписать пререлиз чужого PR), а резолвится по доверенному `workflow_run.head_sha` через API. Публикацию для форков гейтит штатное одобрение прогона мейнтейнером (`approval_policy: all_external_contributors`), которое GitHub требует заново на каждый push: без него сборка не доходит до `conclusion == success`. Cleanup переведён на `pull_request_target`, поскольку при закрытии fork-PR токен точно так же read-only.
- **Known limitations:** гейт публикации живёт **вне репозитория** — это настройка Actions `approval_policy`, которая должна иметь значение `all_external_contributors`; проверяется через `gh api repos/gentslava/elektronny-gorod/actions/permissions/fork-pr-contributor-approval`. Ни один файл репозитория не сломается и не предупредит, если её понизят: при дефолтном для публичных репозиториев `first_time_contributors` возвращающийся внешний контрибьютор получает прогон без одобрения, тот доходит до `conclusion: success`, и сборка публикуется автоматически. Одобрение прогона — общая кнопка на все workflow PR, поэтому решение «прогнать тесты» и решение «раздать сборку пользователям HACS» не разделены. Состояние PR перепроверяется непосредственно перед созданием релиза. Экшены закреплены мажорными тегами, а не SHA: решение владельца, поскольку пины без ручного обслуживания фиксируют и будущие уязвимости; цена — доверие к переносимому тегу в job с `contents: write`. Релиз пересоздаётся (delete + create), поэтому при сбое создания PR временно остаётся без пререлиза.
...
- **Status:** ✅ **RESOLVED** — `1607abb` (PR #95); откат — прямой правкой `master`; повторная заявка после закрытия обоих правил — ветка `feat/silver-coverage`.
...
- **Урок, записанный отдельно:** оба отзыва произошли по одной причине — evidence одного правила подставлялся под другое. В первый раз `config-flow-test-coverage` под `test-coverage`, во второй `docs-actions` под `action-setup`. Проверка заявки уровня должна начинаться с чтения текста правила, а не с поиска подходящей цифры.
...
- **Чем закончилось:** заявка поднималась дважды и дважды отзывалась, прежде чем стала верной. Второй отзыв: `ha-expert` показал по практике ревью ядра (PR #162463, #149736), что «above 95% **for all integration modules**» читается помодульно, а у нас 14 модулей были ниже порога при агрегате 95.02%. Тогда же QA нашёл, что заявка держалась на тесте без единого утверждения: снятие `test_every_method_guards_the_response_type` опускало покрытие до 94.77%. Итог — покрытие доведено так, что оговорка не нужна: **все 42 модуля выше 95%**. Чтобы заявка не держалась на разовом замере, порог закреплён в CI отдельным шагом, и он помодульный: агрегатного было бы мало — модуль, упавший до нуля, прячется за общей цифрой (ADR-0010 D-05). Живые цифры — в [`testing/strategy.md`](../testing/strategy.md), единственном их источнике.
- **Что было сделано до отзыва:** пять правил Silver закрыты по-настоящему и откатом не затронуты — переавторизация ([A-108](project-audit.md)), покрытие config flow ([A-107](project-audit.md)), `parallel-updates`, `log-when-unavailable`, документация. Плюс `runtime-data`: координатор переехал в `entry.runtime_data` с типизированным алиасом.
...
- **Как закрыто:** оба действия отвечают `ServiceValidationError` с переведённой причиной, когда отвечать или завершать нечего. Регистрация действий переехала в `async_setup` (правило Bronze `action-setup`): действие обязано существовать даже при незагруженной записи, иначе автоматизация падает с «сервис не найден» и человек ищет ошибку не там. Тем же классом закрыты переключатели «не беспокоить»: раньше отказ оператора уходил в лог, а тумблер молча возвращался.
- **Регрессия, найденная ревью и снятая:** первый вариант `hangup` отбирал контроллеры по `current_call()`. Этот признак гаснет по истечении **окна ответа**, а разговор живёт дальше — живой звонок с открытым микрофоном отказывался завершаться и держался до 120-секундной страховки. Теперь решение принимает сам контроллер: `async_hangup()` сообщает, было ли что снимать.
- **Вторая регрессия, найденная ревью и снятая:** переезд регистрации в `async_setup` был сделан без парной правки `async_unload_entry`, где действия снимались при выгрузке последней записи. `async_setup` компонента HA зовёт один раз за запуск (домен остаётся в `hass.config.components`), поэтому после первого же reload — смена опций, переавторизация, кнопка «Перезагрузить» или саморелоад миграции видимости — оба действия исчезали до перезапуска HA. Правило `action-setup` существует ровно затем, чтобы действие было даже при незагруженной записи; получилось наоборот. Независимо воспроизведено `code-reviewer` и `ha-expert`.
...
- **Почему это дефект:** автоматизация «открыть и впустить» не отличала открытую дверь от закрытой, а через две секунды исчезал и визуальный след. Тот же класс, что `answer`/`hangup` в [A-110](project-audit.md): предыдущий круг закрыл `lock.lock` — действие, которое не может сработать в принципе, — и пропустил то, которое реально отказывает.
- **Как закрыто:** состояние `JAMMED` остаётся (оно информативно), но действие поднимает `HomeAssistantError` с ключом `cannot_unlock` и переводом на оба языка.
...
- **Почему это дефект:** правило Silver `log-when-unavailable` требует одной записи о пропаже и одной о возвращении. Дедупликация в `_note_failure` покрывала только подзапросы по адресам; путь «оператор недоступен целиком» — тот самый, о котором правило, — не покрывала.
- **Как закрыто:** своего лога на этом пути нет вовсе — ядро само пишет об отказе один раз на переходе и о возвращении данных (`Error fetching … data` / `Fetching … data recovered`), так что любая своя строка была дублем. Наружу уходит тип исключения, а не текст. Тем же фронтом ограничена жалоба на пустой список адресов: у заблокированного аккаунта он пуст сутками.
- **Первая правка была неполной:** `code-reviewer` показал, что убран был трейсбек, а не объём. Слоем ниже, в `http.py`, стоял `LOGGER.error` на каждый неуспешный ответ REST — те же 288 записей в сутки на каждый endpoint, и они же сводили на нет дедупликацию `_note_failure` для подзапросов. Строка понижена до `debug` по тому же основанию, что уже было записано рядом для бинарной ветки: транспорт не знает, значим ли отказ, — решает вызывающий. Тогда же нашлось, что на `debug` URL auth-запроса печатался целиком, с телефоном в пути, хотя заголовки и признак тела та же функция уже прятала; URL пропущен через `redact_path`.
- **Известные хвосты, вынесенные за кандидат.** История событий (`history.py`) свои отказы пишет на `debug` и наружу их не поднимает: с возвратом транспорта к `debug` устойчивый отказ операторских event-эндпоинтов следа выше `debug` не оставляет. Достижимость сервиса при этом закрывает не этот фронт, а ядро (`Error fetching … data` на переходе); история же вспомогательная, поэтому правило Silver это не рушит — но пункт заведён осознанно, а не пропущен. Там же: восстановление видимости на старте (`_sync_visibility`) работает по снимку первого обновления, и если настройки экранов в этот момент недоступны или пришли неожиданной формой, скрытые интеграцией сущности показываются снова; поведение то же, что и до ветки. Неожиданная форма отмечается на `debug` — что именно не так, без тела (раздел в префиксе — тот, который спрашивали: для дрейфа корня строка приходит по разу на каждый запрошенный раздел); на уровне выше о ней сказать нечего, для человека это неотличимо от «ничего не скрыто». На фронт «Настройки экранов» садится только та, на которой разбор бросает (неитерируемое вместо списка): фронт владеет этим ответом, и там жалоба уже обычная.
- **Осознанная плата за разведённые фронты:** вложенность допускает пару строк «Камеры места: нет данных» и «Сборка камер: данные снова приходят» в одном цикле — разные фронты про разные факты. Имена разведены нарочно: пока внешний фронт назывался просто «Камеры», в журнале обе строки читались как об одном и том же. Слить их обратно значило бы вернуть то, ради чего они и разведены: отказ по общедомовым гасил бы камеру домофона у двери.
...
- **Root cause:** каждый переключатель строит полный payload из трёх пунктов, читая `coordinator.data`, а `async_set_dnd` снимок не трогала. Дефект держался на двух разных механизмах сразу, и первое объяснение было неверным в обе стороны: дебаунсер `async_request_refresh` первый вызов в окне выполняет **немедленно**, так что после первого переключения снимок как раз перечитывается у оператора, — устаревшие данные получают все последующие, отложенные на десять секунд. И сериализация действий тут не «ни при чём»: без неё все три переключателя строят payload из одного снимка одновременно, и записывать в него уже поздно. Механизм перепроверен по `update_coordinator.py` после находки `code-reviewer`.
- **Как закрыто:** обе половины — `async_set_dnd` записывает принятый payload в снимок и уведомляет подписчиков (иначе состояние в интерфейсе отстаёт до следующего обновления, и тумблер на глазах отскакивает назад), и сохранена сериализация действий. Закреплено на уровне действия: `test_turning_on_every_switch_of_a_place_keeps_all_three` падает при снятии любой из двух половин — до этого теста снятие `PARALLEL_UPDATES` переживал весь набор.
...
- **Как закрыто:** таймаут теперь проваливает тест, а от отменённого владельца требуется именно `CancelledError`.
- **Что выяснилось попутно:** одиночные мутации этот тест не роняют не потому, что он слаб, а потому что инвариант защищён дважды — `except BaseException` разрешает future отменой владельца, а `finally` страхует сверху. Тест ловит снятие обоих механизмов сразу; прежняя версия не ловила и этого.
...
- **Почему это дефект, а не недосмотр покрытия:** поле в форме подписано «номер телефона или договора», формат не навязан и не проверяется, так что вводят как привычно. Строка пишется до отправки запроса — неудача запроса от утечки не спасает.
- **Как закрыто:** дублёр ответа зеркалит запрошенный URL, и существующее утверждение начало покрывать третью строку (мутация теперь роняет тест); маска закрывает сегмент целиком (`[^/?#]+`), проверено на шести формах записи номера.
- **Смежное, оттуда же:** после понижения лога транспорта до `debug` неудачная отвязка push-токена при удалении записи осталась вовсе без сигнала — метод отказ глотает, а вызывающий возврат игнорировал. Токен, оставшийся у оператора, — это push-и на устройство, которое интеграцию уже удалило. Теперь это `warning`.
- **Понижение оказалось слишком широким, и это тоже нашло ревью:** методы вроде `query_cameras` отказ глотают и возвращают пустой список, так что молчащий транспорт оставлял исчезнувшие камеры вообще без следа в журнале — на любом уровне. Правило требует одной строки на пропажу, а не нуля.
- **Промежуточная попытка была неверной и отозвана.** Сначала я завёл жалобу по фронту прямо в транспорте — `warning` на первый отказ пары «путь + статус». `ha-expert` и `security-auditor` независимо показали, что ключ по пути не работает в обе стороны: места, различающиеся только query-строкой (`finance?placeId=`), схлопываются в один ключ, и при двух местах успех одного чистит отметку другого — те же 288 записей в сутки; а на endpoint-ах с идентификатором в пути (архивная запись) ключ, наоборот, размножается, и `warning` приходит на каждый штатный «клип старше срока хранения». Транспорт вернулся к `debug`.
- **Как закрыто на самом деле (для данных координатора; история — отдельный хвост, записан в [A-112](project-audit.md)):** отказ перестали глотать в `query_cameras`, `query_public_cameras`, `query_screens_settings` и `query_dnd_settings` — пустой ответ оператора и его молчание разные вещи, и различить их может только вызывающий. Координатор уже логировал по фронту с правильной гранулярностью «вид данных + место», просто до него отказ не доходил. Источники камер разведены под отдельные фронты, чтобы отказ по общедомовым не уносил камеру домофона у двери.

### A-117. Метка времени при входе была местным временем с подписью `Z`

- **Status:** ✅ **RESOLVED** — `56aa225`.
- **Severity:** **P3** — расхождение с приложением, сегодня терпимое оператором.
- **Area:** `time.py`, вызывается из `config_flow.py` при входе по паролю.
- **Evidence (2026-09-07):** `Time.get_timestamp()` брал `datetime.now()` — наивное местное время — и приписывал `Z`, то есть выдавал за UTC. У абонента в Новосибирске (домашний город оператора) метка уходила на семь часов вперёд настоящего UTC.
- **Почему не ломалось:** оператор выводит компактную метку из полученной ISO и по ней проверяет `hash2`. Обе метки брались из одного момента, поэтому врали одинаково и сходились между собой. Ломается это в тот день, когда оператор начнёт проверять окно давности.
- **Чем доказано расхождение:** три HAR-снимка, два устройства в разных часовых зонах. В снимке, снятом прокси в UTC, метка совпадает с временем отправки; в двух снимках с устройства в `+07` метка ровно на семь часов позади времени отправки. То есть приложение везде шлёт настоящий UTC, а не местное время. Все три входа получили `200` с токеном — оператор такую метку принимает. Точные значения намеренно не приводятся: это моменты входа из живых сессий владельца, а сами снимки не зря лежат вне репозитория. Проверить можно по `research/api/` — сверить поле `timestamp` тела запроса с заголовком `Date` его ответа: заголовок генерирует сам оператор, поэтому он не зависит ни от часов прокси, ни от зоны устройства. Во всех трёх снимках расхождение 0–2 секунды. Сверка с `startedDateTime` записи даёт тот же вывод, но слабее: она фиксирует зону прокси, а не устройства, и потому не отличает «приложение шлёт UTC» от «устройство стояло в UTC». Мы это поведение зеркалим — [ADR-0006](../decisions/0006-mirror-app-behavior.md).
- **Второй дефект, найденный по дороге:** метка собиралась срезом трёх символов с конца `isoformat()`. При ровной секунде доли секунды не печатаются вовсе, и срез отрезал хвост самих секунд — примерно один вход из миллиона уходил с покалеченной меткой. Заменено на `timespec="milliseconds"`.
- **Почему нашлось только сейчас:** в роадмапе пункт стоял строкой «A-31 UTC в `time.py`» без единого слова о том, что доказательства лежат в этом же репозитории. Тестов на модуль не было ни одного.

### A-116. Отказ настроек видимости на старте стирает пользовательский выбор

- **Status:** 🔴 OPEN.
- **Severity:** **P3** — потеря настройки, которую нечем восстановить.
- **Area:** `__init__.py:_sync_visibility`, `coordinator.py` (фронт «Настройки экранов»).
- **Evidence (2026-09-07):** найдено `ha-expert` при аттестации PR #96, механизм перепроверен по коду. Если `/settings/screens` недоступен в момент загрузки записи, фронт подставляет пустые множества скрытого, и для каждой сущности `api_hidden` становится `False`. Дальше в `_sync_visibility` за **один проход** срабатывают две ветки подряд: `not api_hidden and user_shown` снимает флаг `user_shown`, а следующая `elif not api_hidden` снимает `we_set_integration`. Оба признака стёрты.
- **Почему это дефект:** `user_shown` — это записанный факт «человек нажал «Показывать на панели» у сущности, которую спрятала интеграция». После стирания он неотличим от «никогда не нажимал»: когда настройки вернутся и оператор снова отдаст сущность скрытой, интеграция спрячет её заново. Выбор человека восстановить нечем — он нигде больше не хранится.
- **Смежная половина, записанная раньше:** сущности, скрытые интеграцией и **не** переопределённые человеком, на том же проходе показываются снова. Это заметно, но само чинится следующим успешным обновлением; потеря override — нет.
- **Чего это не касается:** скрытие, выставленное человеком вручную (`hidden_by=USER`), не трогается — снимается только своё.
- **Не регрессия ветки `feat/silver-coverage`:** до неё `query_screens_settings` возвращала `{}` на любой отказ, то есть тот же вход и тот же результат. Ветка добавила лишь узкий новый триггер (неразбираемая форма ответа) и при этом заменила строго худшее поведение: раньше разбор стоял вне защищённого блока, и дрейф схемы ронял всё обновление, а запись вообще не загружалась.
- **Куда копать:** различать «оператор сказал, что ничего не скрыто» и «мы не знаем, что скрыто». Второе не должно приводить к записи в реестр вовсе — ни к снятию скрытия, ни к стиранию флагов.
...
- **Fix:** токен убран в свёрнутую секцию формы (`data_entry_flow.section`) — так ядро и предлагает заменять этот механизм, и так уже сделано в самом ядре (`androidtv`, `asuswrt` и другие). Обычный пользователь снова видит одно поле, опытный разворачивает второе, и от настроек аккаунта это больше не зависит. Первым порывом было просто снять условие и показать оба поля всем, но это потеряло бы исходный замысел — на что владелец и указал. Тест `test_config_flow_avoids_deprecated_core_api` не даёт свойству вернуться.
...
- **Смежные находки:** причина отказа `no_contracts` и ключ ошибки `invalid_contract` выставлялись кодом, но перевода не имели — пользователь увидел бы сырой ключ вместо текста. Обе добавлены в три файла переводов. `test_flow_messages_are_translated` проверяет теперь и отказы, и ошибки; проверено мутацией — удаление любого перевода роняет тест. Ключ `invalid_access_token` наоборот стал мёртвым (пустое поле токена теперь означает «не заполнено») и удалён.
...
- **Status:** ✅ **RESOLVED** — `0673aed` (PR #89). Доработка — `8c6deb5` (PR #90).
- **Severity:** **P2** — заметная задержка на каждом открытии карточки.
- **Area:** `camera.py` (`async_camera_image`).
- **Evidence:** пользовательское наблюдение (2026-09-05): «открыл — белый экран, загрузилось, пошло видео; закрыл, снова открыл — снова белый экран». Кадр не переиспользовался между открытиями.
- **Root cause:** `async_camera_image` шёл в `/rest/v1/forpost/cameras/{id}/snapshots` на каждый вызов. Поле `_image` существовало, но работало лишь как запасной кадр на случай пустого ответа — ни TTL, ни переиспользования между открытиями не было. HA зовёт этот метод при каждом рендере превью и как постер перед запуском потока, поэтому карточка ждала операторский JPEG, показывая пустое место.
- **Fix:** stale-while-revalidate с потолком. Свежий кадр (моложе `SNAPSHOT_FRESH_SECONDS`) отдаётся из памяти; устаревший тоже отдаётся немедленно, а обновление уходит в фоновую задачу записи. Старше `SNAPSHOT_MAX_STALE_SECONDS` кадр за текущий уже не выдаётся — иначе `camera.snapshot` в автоматизации и постер экрана вызова, которые не поллят камеру, приложили бы вид «с момента последнего просмотра». Экран входящего вызова вообще ходит мимо кэша: гость стоит у двери сейчас, и приложение оператора в этот момент тоже перезапрашивает снимок. Ключ — нормализованный размер, тот самый, что уходит оператору. Параллельные вызовы объединяются на обоих путях: и холодном, и фоновом.
- **Смежный дефект (Critical, найден ревью):** бинарный путь `http.py` отдавал тело **любого** ответа, не проверяя статус. При отказе оператора 500 приходила непустыми байтами, проходила проверку «кадр получен» и попадала в кэш: исправный кадр уничтожался, а ошибка показывалась как превью весь TTL и переотравлялась при каждом обновлении. До кэша та же 500 портила один рендер и следующий запрос восстанавливал картинку — то есть кэш без этой проверки делал хуже, чем было. Исправлено в корне (статус на бинарном пути) плюс проверка JPEG-сигнатуры перед записью в кэш.
- **Tests:** снимочная секция `tests/test_camera_auto_recovery.py` — 32 теста (35 кейсов с параметризацией): кэш, немедленная отдача устаревшего, потолок устаревания, тело ошибки оператора вместо кадра, отказ в фоне, первый кадр без фонового дубля, одновременное первое открытие, дедупликация (в том числе при отмене ожидания), ключ по нормализованному размеру, скрытая и недоступная камера при прогретом кэше, правило размера на обоих путях, пауза на холодном и фоновом пути, отмена фонового фетча при удалении сущности, якоря на окна 30 с и 5 мин с обеих сторон, принадлежность операторского запроса записи, выбор свежайшего годного кадра и вытеснение старейшего, одна строка в журнале на отказ вместо строки на рендер. Проверка статуса на бинарном пути покрыта в `tests/test_http.py` — то есть в корне, а не только второй линией обороны. Плюс `tests/test_call_camera.py` — экран вызова ходит мимо кэша и не гоняет оператора на дефолтном интервале MJPEG. Фоновое обновление ожидается напрямую через задачу: `async_block_till_done()` фоновые задачи записи не ждёт, и зелёный прогон держался бы лишь на том, что мок резолвится без приостановки.
- **Доработано по прод-логам (2026-09-05, тест PR на живой системе):** белый экран воспроизводился и после кэша, потому что HA просит кадр в разных размерах — в журнале `80x80`, `300x169`, `390x219`, `490x276`, `1024x576` для списка, карточки и полноэкранного вида. Ключ по размеру означал холодный старт на каждом новом размере. Теперь, если кадра нужного размера нет, отдаётся имеющийся — но только не мельче запрошенного. Уменьшение безвредно, ядро масштабирует JPEG само; увеличение портит картинку, и подстановка иконки `80x80` на большую карточку давала не «чуть хуже», а нечитаемое месиво (замечено владельцем на скриншоте дашборда). Нужный размер догружается в фоне.
- **Пауза при отказе:** оператор отвечает `531` на `/snapshots` тех же трёх камер, что не отдают видео. Без паузы каждый рендер карточки бил в API, не давая кадра. Введён backoff с потолком; показатель ограничивается до возведения в степень (урок A-104). Первый успешный кадр сбрасывает счётчик.
- **Окна подобраны по проду:** свежесть 30 с (карточка HA опрашивает камеру примерно раз в 10 с — при равном интервале почти каждый опрос попадал в «протух» и порождал запрос), потолок устаревания 5 минут. Минуты не хватало: страницу закрывают и возвращаются позже, фоновой кадр к тому моменту протухал, запрос был потрачен впустую, а пользователя снова встречал белый экран. Потребителям, которым нужен именно текущий вид, потолок не помощник — экран вызова ходит мимо кэша.
- **Известное ограничение:** кадр, загруженный в фоне, доедет до открытой страницы не мгновенно, а на следующем опросе карточки (порядка десяти секунд). Заставить браузер перезапросить картинку интеграция не может: `entity_picture` не меняется, а запись состояния перерисовку не вызывает.
- **Второй круг ревью (2026-09-05, четыре профиля):** три вердикта блокирующих. Закрыто по существу: (1) при отказе оператора кадр возвращался мимо обоих новых правил — и мимо потолка устаревания, и мимо запрета растягивать мелкий кадр, то есть на карточку выдавалась суточная иконка ровно в том режиме, ради которого ветка писалась; (2) пауза при отказе стояла только на холодном пути, а прод-триггер наступает на прогретом — открытая карточка перерисовывается каждые несколько секунд и каждый раз уходила к оператору (замерено: 10 запросов вместо 1); (3) запись о выполняемом запросе снималась в `finally` ожидающего кадра, а ядро рвёт ожидание снимка через 10 с при 60-секундном запросе — медленный оператор получал вторую параллельную сессию; (4) словарь кэша не вытеснялся никогда, ключ приходит из query-строки `/api/camera_proxy`; (5) операторский запрос принадлежал `hass`, а не записи, и переживал выгрузку; (6) MJPEG-фолбэк экрана вызова наследовал дефолтные 0.5 с ядра, а его кадр идёт мимо кэша и мимо паузы — две операторские сессии в секунду на всё время разговора.
- **Отклонено с обоснованием:** (а) кламп размера кадра из query-строки — перебор размеров не даёт держателю image-токена ничего, чего у него нет и без перебора, а невалидная ширина уходила в URL оператора и до этой ветки; память ограничена бюджетом кэша, и это единственный дефект, который ветка действительно внесла; (б) отдельный backoff на каждый размер — оператор отдаёт `531` на весь `/snapshots` камеры, поэтому пауза per-camera и есть верная гранулярность, а per-size умножила бы запросы в реальном отказе; комментарий уточнён; (в) снятие `getattr`-фолбэка в `call_camera` — это ровно тот класс правки, против которого написан A-106; (г) `frame_interval` обычной камеры не влияет на нагрузку оператора вовсе — её гейтит кэш, а не тик лупа, поэтому величина интервала здесь вопрос свежести показанного кадра, а не числа запросов (у экрана вызова наоборот: он ходит мимо кэша, там интервал определяет нагрузку напрямую).

- **Третий круг ревью (2026-09-05):** security, HA и code-review закрыли candidate, QA вернул одну Important. Экран вызова при отказе оператора подставлял кадр из кэша до пяти минут давности, хотя докстринг, комментарий к константе и CHANGELOG обещали «всегда живой»; обе ветки поведения при этом были зелёными, то есть контракт не проверялся ничем. Разбор показал, что фолбэк внутри фетча перепроверял ровно то, что вызывающий с карточки уже нашёл пустым (до запроса доходят, только исчерпав и точный размер, и подстановку), — то есть для дашборда он не давал ничего, кроме одного кадра в узком окне, когда за время ожидания кэш успевал наполниться запросом другого размера; единственным его живым потребителем оказался путь, которому он противопоказан. Цена этого окна — один кадр до следующего опроса карточки, и тот же коммит сократил этот опрос с 30 до 10 секунд. Фолбэк удалён целиком; вместе с ним ушли поле `_image`, ставшее write-only, и выбор более старого кадра при наличии более свежего годного. Дополнительно: интервал MJPEG совпадал с окном свежести, а ядро отмеряет период до запроса — обновление срабатывало через тик, и половина показанных кадров была минутной давности при неизменной нагрузке; интервал взят в треть окна. Отказ снимка перестал быть виден на дефолтном уровне логирования — возвращена одна строка по фронту отказа и одна по восстановлению.
- **Окружение (устранено после merge):** репозиторный `.venv` содержал HA `2026.2.3` и Python 3.13 при объявленном минимуме `2026.8.1` и Python 3.14, поэтому команда, записанная как определение гейта `TESTS_PASS`, падала на любом коммите — 128 тестов на head ветки, 98 на base `0673aed`. Окружение пересобрано под пин `PHC_CURRENT`; туда же поставлен `pyright`, а `pyrightconfig.json` теперь указывает на `.venv`, иначе анализ молча брал первый `python` из PATH.
- **Известное ограничение (не регрессия ветки, воспроизведено security-review):** `?width=` из нескольких сотен цифр роняет `OverflowError` на `w / 16` в `camera.py` и `coordinator.py` — 500 и traceback в журнале на запрос. Тот же путь существует на `master`; вреда сверх траты запроса нет, секретов в трейсе нет.
- **Follow-up:** первое открытие камеры по-прежнему ждёт оператора — показать нечего. Прогрев кадров при старте не делается намеренно: это N параллельных запросов на каждую загрузку HA, а оператор и без того в отказе (A-102).

### A-104. `OverflowError` в расчёте паузы ронял открытие всех камер

- **Status:** ✅ **RESOLVED** — `d539337` (PR #86).
- **Severity:** **P1** — не открывается ни одна камера, включая исправные.
- **Area:** `stream_manager.py` (`_record_failure`), `camera.py` (`_recovery_cooldown`).
- **Evidence (прод, 2026-09-05):** `OverflowError: int too large to convert to float` из `_record_failure`, вызванного из `async_refresh`. Исключение уходило в `stream_source()`, поэтому HA получал ошибку вместо URL — в журнале следом `Error from stream worker: Error demuxing stream while finding first packet` для трёх лифтовых камер.
- **Root cause:** потолок `min()` применялся **после** возведения в степень, то есть `2 ** failure_count` вычислялось целиком. Счётчик неудач ничем не ограничен, а оператор отдаёт `500` сутками (см. A-102), поэтому примерно после 1024 неудач подряд умножение перестаёт помещаться во float. На пятиминутном шаге это около трёх с половиной суток непрерывного отказа.
- **Impact:** камеры не открываются вовсе. Счётчик живёт в памяти, поэтому перезапуск HA возвращает работоспособность — до следующего затяжного отказа.
- **Fix:** показатель степени ограничивается **до** возведения, предел выведен из самих констант. Та же форма нашлась и в `camera.py` — независимое ревью показало, что бэкофф из A-102 воспроизвёл дефект дословно; там последствие тяжелее, потому что исключение летит из `@callback`, который зовут HA Stream и оба таймера, и авто-recovery для камеры умирала бы насовсем.
- **Tests:** `test_retry_delay_survives_long_operator_outage` и параметризованный `test_backoff_is_capped` прогоняют счётчики до 100 000. Мутация «вернуть потолок после возведения в степень» роняет оба.
- **Урок:** один и тот же паттерн был воспроизведён в соседнем модуле через час после исправления. Клампить показатель, а не результат — и проверять границу, а не удобное маленькое значение: прежний `test_backoff_is_capped` пинил 99 и проходил с дефектом.

### A-102. Recovery потока долбит оператора при устойчивом отказе

- **Status:** ✅ **RESOLVED** — `fa93c14` (PR #87).
...
- **Fix:** счётчик неудач подряд, пауза удваивается с потолком 30 минут (показатель ограничивается до возведения в степень — см. A-104), сброс при первом успехе. Ручное открытие камеры backoff не затрагивает — `stream_source()` идёт своим путём и всегда пробует заново.
- **Границы эффекта:** гасится реактивный путь через health-poll. При включённом `go2rtc_keep_warm` — а это конфигурация инцидента — независимый планировщик `stream_manager._record_failure` продолжает раз в 300 с обновлять источник, поэтому суточное число обращений падает примерно с 1400 до 340 на камеру, а не до нуля. Кроме того, отказ самого go2rtc возвращает непустой URL прямого оператора, и такой исход считается успехом: backoff покрывает отказ оператора, но не отказ go2rtc.
...
### A-103. Версия на сайте поднималась вручную и отставала от релиза

- **Status:** ✅ **RESOLVED** — `88e352d` (PR #88).
- **Severity:** **P2** — публичный сайт вводит пользователя в заблуждение.
- **Area:** `website/index.html`, `website/src/data/project.ts`, `website/vite.config.ts`, `.github/workflows/website.yml`, [`release.md`](../aidd/runbooks/release.md).
- **Evidence:** релиз 4.1.0 вышел 2026-09-02, а сайт до 2026-09-05 показывал `4.0.1` в JSON-LD и в чипе шапки и вёл на release notes прошлого релиза. Обнаружено случайно, при разборе A-100; тем же порядком сайт больше полугода обещал минимум HA `2024.10.4`.
- **Root cause:** версия и минимальная HA дублировались в трёх местах (`project.ts`, JSON-LD, чип), а поднимались руками. Релизная процедура трогала сайт только «если менялся `website/**`», то есть при обычном релизе интеграции его никто не обновлял. Проверка совпадения, добавленная в A-100, ловила расхождение постфактум, но не мешала ему возникнуть.
- **Fix:** дублирования больше нет. `project.ts` читает `manifest.json` и `hacs.json`, разметку заполняет Vite при сборке через плейсхолдеры `%APP_VERSION%` / `%MIN_HA%`, ссылка на release notes собирается из версии манифеста. Деплой сайта триггерится и на `manifest.json`/`hacs.json` — иначе подъём версии не доходил бы до опубликованной страницы. Релизный чеклист требует прогона и деплоя сайта на каждом релизе, а не только при правках его исходников.
- **Tests:** `website/test/sync.test.ts` переориентирован: вместо сверки равных по построению значений он стережёт, чтобы константу не захардкодили обратно, и что файл release notes для текущей версии написан. Проверено мутациями: возврат хардкода в JSON-LD, возврат хардкода в `project.ts` и ненаписанный файл релиза — каждая роняет прогон.

### A-100. Камеры и замки пропали из HA после обновления ядра до 2026.9

- **Status:** ✅ **RESOLVED** — `8111d23` (PR #85).
- **Severity:** **P0** — часть сущностей не создаётся вовсе.
- **Area:** `camera.py`, `event.py`, `lock.py`, `sensor.py`, `__init__.py`, `history.py`, `hacs.json`, CI matrix.
- **Evidence (2026-09-02):** на проде после обновления HA сущности камер и домофонов стали недоступны; в логе — `RuntimeError: Detected code that calls DeviceInfo with via_device` из `homeassistant.helpers.frame`. HA 2026.8 объявил `via_device` устаревшим, а 2026.9 убрал его из `DeviceInfo` (в `async_get_or_create` он принимается до 2027.8), а `frame`-хелпер поднимает `MissingIntegrationFrame` до `RuntimeError`, когда фрейм интеграции не определяется — сущности здесь добавляются из генератора, поэтому предупреждение становится фатальным.
- **Root cause:** `via_device` адресовал родителя по `identifiers`, которые не уникальны между config entries. Замена `via_device_id` требует **готовый** `device_id`, то есть родительское устройство обязано существовать раньше, чем на него сошлётся первая платформа. Порядок forward-а платформ (`camera` раньше `sensor`) при этом против нас: устройство адреса исторически создавалось попутно сенсором баланса.
- **Fix:** новый модуль `device.py` — устройства адресов создаются явно в `async_setup_entry` **до** `async_forward_entry_setups`, а платформы передают готовый `via_device_id` в конструкторы сущностей. Id читается из реестра штатной обёрткой ядра `dr.async_get_device_id_by_identifier(..., config_entry_id=...)`, а не из собственной карты в `hass.data`: entry-scoping обеспечивает само ядро, отдельная очистка при unload не нужна, отступление от Bronze `runtime-data` не расширяется. Минимум HA поднят до `2026.8.1`; совместимость с deprecated-полем сознательно не поддерживается (решение владельца).
- **Migration:** отдельная миграция не нужна — `async_get_or_create` дописывает `via_device_id` существующему устройству по тем же `identifiers`, поэтому установки, обновляющиеся с «плоской» аварийной версии, получают иерархию на первом же старте. Инвариант закреплён тестом `test_flat_device_gets_linked_on_restart`.
- **Tests:** `tests/test_device_hierarchy.py` — 13 тестов. Фикстура шире одного домофона по трём осям, и каждая ось несущая. Два адреса ловят привязку к чужому месту. Вторая точка входа без `externalCameraId` — единственная конфигурация, где устройство создаёт только `lock.py`: `_collect_locks_for_place` заводит замок на каждую точку входа, а `camera`/`event`/`sensor` покрывают лишь подмножества и дедуплицируют по `(place_id, access_control_id)`. Параметр `with_account_history` убирает из записи `account_id`/`subscriber_id`: с ними `event.py` создаёт сущность истории аккаунта, которая попутно регистрирует адрес и маскирует отсутствие фикса, без них адрес не создаёт никто.
- **Mutation evidence (перемерено 2026-09-05 на 13 тестах):** роняют тесты — полный откат `__init__/camera/lock/sensor/event` к до-фиксовому коду (5 на 2026.8.1, 6 на 2026.9 — шестое падение даёт гейт ниже), удаление вызова `async_register_place_devices` (5), потеря связи только в `lock.py` (5), подмена адреса на первый попавшийся (2–3, смотря по ширине мутации), возврат `via_device` в `camera.py` + `lock.py` (1, на 2026.9), запись явного `None` вместо пропуска ключа (1), снятие guard-а пустого `place_id` в `async_register_place_devices` (1). **Не роняет и не должно:** снятие ныне удалённого дублирующего guard-а в `place_device_id` — пустой id и так не находится в реестре.
- **Гейт на сам отказ:** `test_setup_does_not_use_deprecated_via_device` проверяет, что после setup ядро не сообщило об устаревшем `via_device`. Это и есть исходный отказ — в проде сообщение поднималось до `RuntimeError`, потому что фрейм интеграции не определялся; в PHC фрейм определяется, поэтому остаётся предупреждение, по нему и ловим.
- **Почему нужны обе проверки:** они ловят регрессию разными путями, а не разные версии. На 2026.8.1 ловить нечего: `via_device` там поддерживаемый параметр, отказа нет — обе проверки на этом плече пусты. На 2026.9 pyright видит литеральный `via_device=` в типизированном коде, но только его; runtime-гейт срабатывает независимо от того, как собран `DeviceInfo`. Обратная сторона: если `via_device` добавить **рядом** с `via_device_id`, ядро упадёт с `HomeAssistantError` до отчёта об устаревании, и именно этот гейт промолчит. Сюита при этом краснеет — четыре теста иерархии падают на сломанной привязке, — но адресно такую правку ловит pyright. Прежняя формулировка «ловит pyright» была верна лишь наполовину.
- **Follow-up:** до этого PR иерархия устройств не была покрыта ни одним тестом, хотя «всё в устройствах» — заявленное свойство 4.0. Тесты добавлены вместе с фиксом.

### A-101. Статический анализ (pyright/Pylance) не входит ни в один gate

- **Status:** ✅ **RESOLVED** — `8111d23` (PR #85).
- **Severity:** **P2** — латентные дефекты и шум, скрывающий реальные находки.
- **Area:** весь `custom_components/elektronny_gorod/**`, CI.
- **Evidence (2026-09-03):** `pyright 1.1.411` на HA 2026.9 даёт **77 ошибок** в 18 файлах при нуле предупреждений; тот же счёт на `master` — регрессии в текущей ветке нет. Разбивка: `reportArgumentType` 25, `reportIncompatibleVariableOverride` 22, `reportPrivateImportUsage` 16, `reportPossiblyUnboundVariable` 8, `reportOptionalMemberAccess` 2, прочее 4.
- **Impact:** среди находок есть потенциально исполнимые: `http.py:140-147` читает `response`, который остаётся несвязанным, если `method` не входит в `GET`/`POST`/`DELETE` — сейчас недостижимо, но защищено только соглашением вызывающих; `config_flow.py:110` (`phone`) и `:383` (`result`) — того же класса. Основная масса (`reportIncompatibleVariableOverride` на `_attr_*`, `reportPrivateImportUsage` на HA-внутренностях) — известный шум типизации HA, который нужно осознанно подавить, иначе реальные ошибки в нём тонут.
- **Fix:** добавлен `pyrightconfig.json` (`typeCheckingMode: standard`, Python 3.14) и job `pyright` в `python-tests.yaml`; pyright ставится через pip, а не сторонним экшеном с mutable-ref (см. A-98 follow-up). Совпадение версии ядра между job-ом и current-плечом матрицы закреплено `test_pyright_job_types_against_current_matrix_core`: без него статическая половина гейта A-100 могла бы молча начать типизировать против другого ядра. Шум HA-типизации подавлен двумя правилами с обоснованием: `reportPrivateImportUsage` (ядро не экспортирует `DeviceInfo`, `LockState`, `HomeAssistantView`, websocket-хелперы, хотя это публичный API интеграций) и `reportIncompatibleVariableOverride` (интеграции штатно переопределяют `available`, `extra_state_attributes`, `native_value` как property). `reportMissingImports` оставлен на уровне ошибки, а объявленное в `manifest.json` `firebase-messaging` ставится в job: понижать правило нельзя — тогда опечатка в имени модуля проходит гейт, хотя это `ImportError` при загрузке интеграции. Результат: **0 ошибок, 0 предупреждений**.
- **Исправленные дефекты:** `http.py` — последняя ветка выбора метода стала `else`, метод типизирован `Literal`, иначе неизвестный метод уходил дальше с несвязанным `response`; `config_flow.py` — блоки перенесены внутрь своих веток, где `phone` и `result` заведомо связаны (первый случай был достижим при `user_input` без `CONF_PHONE`); `media_source.py` — пятый вызов `_coordinator` остался без `None`-guard; на практике он недостижим (предшествующий `self._camera()` уже возвращает `None` и роняет `BrowseError`), поэтому guard добавлен как защита контракта, а не как исправление наблюдаемого отказа; `go2rtc.py` — сигнатура `headers: dict = None` исправлена на `dict | None`; `sip/manager.py` — явная проверка futures вместо `AttributeError` посреди ожидания; `event.py` — `async_update_entity(**dict)` заменён явными kwargs; `clip_proxy.py` — sentinel-ы стали `Literal`, иначе тип не сужался до кортежа.
- **Остаточные подавления (3, точечные `# pyright: ignore`):** `camera.py:177` — прямые `__init__` базовых классов вместо `super()` из-за значимого порядка инициализации; `camera.py:417` — анализатор не связывает «`result` не None» с «менеджер существует»; `media_source.py:99` — `async_browse_media` возвращает `BrowseMedia`, тогда как база объявляет `BrowseMediaSource`.
- **Follow-up:** сузить возвращаемый тип `async_browse_media` до `BrowseMediaSource` — ядро читает только поля `BrowseMedia`, поэтому рантайм корректен, но это отдельный рефакторинг всего media source с риском для архива.
