...
- **Follow-up:** первое открытие по-прежнему ждёт оператора. Прогрев кадров при старте не делается намеренно: это N параллельных запросов к оператору на каждую загрузку HA, а он и без того отвечает `500` (A-102).
