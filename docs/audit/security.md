...
- **Файл:** `helpers.py:hash_password_timestamp`.
...
- **Файл:** `user_agent.py:UserAgent.__init__`.
...
- **Audience / required control:** новой категории доступа внутри HA не появляется: и `repairs/list_issues`, и `config_entries/get` доступны authenticated HA users, причём второй endpoint уже возвращает `entry.title`. Event обновления Repairs содержит только action/domain/issue ID; новые логи title не выводят. Diagnostics обязательно redacts every `title` key, поэтому user-shared export не переносит resident name/account ID за пределы HA. Release documentation должна отдельно говорить, что Repairs намеренно показывает локальный config- entry title; без этой оговорки или diagnostics redaction S-23 больше не считается принятым.
...
- **Что в журнале:** `entity_id` и `unique_id` каждой сущности, чью видимость меняем. `unique_id` собирается из `place_id` / `access_control_id` / `entrance_id` / camera id. По совокупности строк восстанавливается раскладка видимости, которую человек настроил в приложении оператора.
- **Почему принято:** ADR-0004 и [`no-secret-logs.md`](../../.agents/rules/no-secret-logs.md) относят эти идентификаторы к internal references, а не к PII — они же стоят в обычных REST-URL и не маскируются там. Секретов, токенов и заголовков в этих строках нет. Основной объём на `debug`, то есть без явного включения в журнал не попадает.
...
- **Почему проходит мимо:** сканирование ищет формы токенов, идентификаторов, адресов и телефонов. Посекундная временная метка не подходит ни под одну, поэтому проходит чистой — при том что это поведенческие данные: когда именно человек был дома и аутентифицировался, с точностью до миллисекунды, привязанное к названному устройству.
- **Откуда взялось:** при аттестации A-117 в запись аудита попали три момента входа из живых сессий. `security-auditor` их не отметил (шаблоны не совпали), нашёл `qa-engineer` по смыслу. Вычищено до публикации.
- **Правило:** `.gitignore` на снимках держится ровно до тех пор, пока значения из них не переносят в отслеживаемый файл. Доказательство, снятое со снимка, формулируется отношением и именами полей — не значениями. Имена самих снимков тоже указатели: они раскрывают даты съёмки.
