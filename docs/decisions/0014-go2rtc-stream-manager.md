...
4. Ошибки получают backoff 15/30/60/120/240/300 секунд. Reconcile раз в минуту
   ...
   missing preload, inactive producer и go2rtc restart для текущих камер.
...
10. Cleanup сначала снимает manager preload. Затем получает актуальное число
    ...
    consumers приводит к DELETE. Unload закрывает scheduling, отменяет refresh,
    дожидается уже запущенного reconcile и только затем снимает owned/pending
    preloads.
...
13. Background eligibility повторно проверяется после operator mint и PATCH;
    late background PATCH после policy-off проходит существующий
    consumer-aware cleanup. Дополнительные polling/locking механизмы для
    ненаблюдавшихся interleavings не вводятся.
...
- Hidden `eg_<id>` может появиться при явном просмотре в HA даже с выключенной
  hidden background publication; при включённом main reconcile registration
  cleanup-ится после viewer. Main-off не добавляет отдельный polling loop.
