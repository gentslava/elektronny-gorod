...
Типы Home Assistant берутся из установленного ядра, поэтому анализ не запускается «всухую»: интерпретатор нужно указать явно.
...
`--pythonpath` обязателен: сам по себе pyright берёт интерпретатор из `PATH`, а не из каталога, откуда запущен. В CI это делает `setup-python`.
`firebase-messaging` — требование `manifest.json`; без него анализ спотыкается на импорте в `fcm.py`.

Окружение старее минимума HA даёт ложные ошибки — самая заметная из них «`via_device_id` не является определённым ключом в `DeviceInfo`». Это признак неактуального `.venv`, а не дефекта кода: пересоберите его командой выше.
