...
Last reviewed: 2026-06-24 (feat/intercom-uplink-mic: uplink-микрофон ADR-0013 — HA WS-binary #1, live-прод 2026-06-24, A-85; two-way audio завершён)
...
2026-06-24). Pending merge `feat/intercom-uplink-mic`. PRD —
...
### Реализовано (feat/intercom-uplink-mic — uplink-микрофон, ADR-0013)
...
### Экран вызова: UI + видео-надёжность + мульти-вызов (feat/intercom-call-ui → фазы A/B)

**Готово (feat/intercom-call-ui, pending merge):** карточка `eg-intercom-call-card`
(UI-часть завершена), mic-фикс (auto-start every call + разный текст баннера),
call-camera fixes — снапшот + `available`-only-during-call (2a), не отдавать
мёртвый URL (2b), state-write на смену фазы (регресс 2a), cross-call guard
(чужой `ended` не рвёт активный вызов), anti-delay warm-up стрима на answer.
Три исходных UI-замечания закрыты (слайдер / микрофон / видео рендерится).

- [ ] **Фаза A — видео anti-churn ([A-88](audit/project-audit.md)).** Ветка
  `feat/intercom-video-concurrency`. Видео вызова = copy с общей forpost-камеры
  ...
  teardown стрима+worker на завершении. Экспериментально на проде (откат — из git).
- [ ] **Фаза B — мульти-вызов ([A-89](audit/project-audit.md)).** Отдельная
  ветка, после A. Смена **звонящего домофона**: новый `ring` во время held (не
