Status: Active Owner: Lead Architect Agent Last reviewed: 2026-08-14 (4.0.1 FCM hotfix published and verified; A-97 review lifecycle accepted)
...
- **Платформы:** `camera`, `lock`, `sensor`, `binary_sensor`, `switch`, `event`; realtime FCM/SIP-контроллер и Lovelace-карта вызова.
...
| Integration Quality Scale | ✅ Silver заявлен: все 42 модуля выше 95% покрытия (общий 97.40%), нативная переавторизация, внятный отказ действий, `parallel-updates`, документация параметров и удаления |
...
- Опциональная интеграция go2rtc с real-валидацией; operator-camera source writes консолидированы в PATCH-only client/manager, initial keep-warm активирует dedicated preload consumer.
