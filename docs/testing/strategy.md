...
| Область | Состояние |
|---|---|
| Локальный suite | **819 passed, 2 skipped** (`PYTHONPATH=. .venv/bin/pytest tests/ -q`, 2026-09-05; HA 2026.8.1 и 2026.9). Два skip — тесты FCM, требующие `firebase-messaging`: пакет объявлен в `manifest.json`, но в pytest-окружение не ставится. С ним локально 821 passed. |
...
- [x] `PYTHONPATH=. .venv/bin/pytest tests/ -q` зелёный локально: 819 passed, 2 skipped (2026-09-05).
