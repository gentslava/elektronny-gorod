...
| `log-when-unavailable` | ✅ по фронту, одна строка на пропажу и одна на возвращение, с гранулярностью «вид данных + место»: баланс, домофоны, настройки экранов, камеры места, общедомовые камеры, замки, режим «не беспокоить» и пустой список адресов. О полной недоступности пишет ядро (`Error fetching … data` / `… recovered`), своего лога рядом нет; транспорт об отказах говорит только на `debug` — значимость определяет вызывающий | `coordinator.py:_note_failure`, `_note_success` |
