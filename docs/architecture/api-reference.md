...
Это **один из** потенциальных путей для real-time звонков в HA — поднять
SIP UAC по адресу `sip:<login>@<realm>`, регистрироваться этими
credentials, ловить INVITE. SIP-трафик идёт вне HTTPS (другой транспорт),
поэтому полная картина (что именно приходит через SIP vs WS vs FCM)
требует non-HTTP capture. См. [audit A-49](../audit/project-audit.md).
...
| `CALL_END_ANSWERED_MOBILE` | принят на другом устройстве (тот же `Call-ID`) | `ended` |
