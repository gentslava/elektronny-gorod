...
**✅ Подтверждено probe вживую (2026-06-23):** имитация раздумий **12с** → `REGISTER`
→ `INVITE` → `200 OK` **мгновенно** → latching (downlink растёт, локальный SDP без
STUN) → разговор. Ответ через 12с работает **все разы**; сброс домофоном
обрабатывается, следующий звонок — снова 12с. **Воспроизводит pcap реального
приложения** — модель доказана и захватом приложения, и нашим прототипом.
