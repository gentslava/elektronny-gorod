...
**Architecture:** SIP-UAS внутри интеграции (на базе `voip-utils` из HA core + наш слой G.711/REGISTER/STUN из доказанного `probe`) принимает INVITE, держит RTP G.711 за NAT (STUN + symmetric latching). Downlink/uplink мостится в go2rtc (`exec`-backchannel) → готовая WebRTC-карта в браузере = трубка. См. [design.md](design.md).

**Tech Stack:** Python 3.13 (asyncio), `voip-utils` (Apache-2.0), `audioop-lts` (G.711 на py3.13), go2rtc (`exec`-backchannel), pytest + pytest-homeassistant-custom-component.
...
| `custom_components/elektronny_gorod/manifest.json` | requirements: `voip-utils`, `audioop-lts` | модифицируется (Task 2) |
...
- [ ] **Step 1: Добавить зависимости в manifest и установить**
...
```bash
.venv/bin/pip install "audioop-lts>=0.2.1" "voip-utils>=0.3.4"
```

(Точные min-версии — из исхода Task 1; здесь актуальные на 2026-06.)
...
def test_g711_roundtrip_is_idempotent(pt: int) -> None:
    # G.711 lossy, но encode(decode(g711)) идемпотентен на закодированных байтах —
    # классическое свойство µ-law/A-law (стабильность quantization-уровней).
    g711 = bytes(range(256))  # все возможные кодовые точки
    pcm = g711_to_pcm(g711, pt)
    assert pcm_to_g711(pcm, pt) == g711
...
- `sip/session.py` — состояние диалога (Call-ID, tags, Record-Route, remote contact) + BYE.
- `sip/protocol.py` — REGISTER (Digest MD5, из probe `digest_response`) + приём INVITE + 200 OK с SDP-answer (на базе `voip-utils` либо asyncio — по D1).
- SDP parse/build для G.711 (если `voip-utils` не покрывает — по D1; иначе переиспользуем).
- `SipManager` (фасад: `async_answer()`, `async_hangup()`, state) — интегрируется в `__init__.py` lifecycle, стартует transient-register по D2.
- Тесты: Digest golden-vector, SDP parse, диалог-state для BYE.
