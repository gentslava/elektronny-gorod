...
**Решение D2 (финал, 2 live-теста 2026-06-23): HELD-регистрация + ответ по явному
действию. «Занято» — артефакт авто-ответа, не регистрации.**

Гипотезу «приложение не держит регистрацию (push-wake)» проверили двумя пробами:

| Тест | Конфигурация | Результат |
|---|---|---|
| **1. push-to-register** | НЕ держим рег.; по FCM `CALL_INCOMING` → transient `REGISTER` (за 154мс) → ждём INVITE | ❌ **INVITE не пришёл** за 20с — forking прошёл без нас; голый push-to-register не работает |
| **2. held без авто-ответа** | держим рег.; на INVITE → `180 Ringing` (НЕ `200 OK`) | ✅ **INVITE пришёл** (3 ретрансмита); **телефон принял вызов штатно**; панель НЕ «занято» |

**Выводы:**
- Push-wake-модель приложения опирается на **предварительный** push-binding на стороне
  Kazoo (RFC 8599-подобный): без него поздняя регистрация вызов не ловит (тест 1).
  Приложение использует push-wake из-за **мобильных ограничений** (батарея, iOS/Android
  убивают сокет) — HA-сервер always-on в них **не нуждается**.
- **«Занято» (N2) = артефакт авто-ответа**, НЕ held-регистрации. Held + `180 Ringing`
  (не забираем) → телефон работает нормально, вызов проходит, линия свободна (тест 2).

**→ Финальная стратегия для интеграции:**
- SIP-канал держим зарегистрированным **постоянно** (re-REGISTER keep-alive) — HA
  always-on, это нормально и доказано не вредит.
- На входящий `INVITE` — **НЕ авто-отвечаем**: показываем событие (event-сущность есть),
  `180 Ringing`; `200 OK` шлём **только по явному «ответить»** пользователя в окне
  `CallInvalidated` (~30с). Это снимает «Занято» и сохраняет first-answer-wins
  (телефон тоже звонит — кто первый ответил, забрал).
- Push-wake (RFC 8599) — **не требуется** (held проще и доказан). Точный mirror
  регистрации приложения — возможная будущая оптимизация, не нужная для работы.
