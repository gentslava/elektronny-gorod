...
**Решение D2 — стратегия регистрации = HELD (постоянная), НЕ transient-by-FCM:**
- Transient-register по FCM **невозможна**: к моменту прихода FCM `INVITE` уже пришёл
  (опережает на ~0.1с) — нет окна, чтобы зарегистрироваться по сигналу и успеть.
- → SIP-канал держим зарегистрированным **постоянно** (как `probe`: re-REGISTER
  по таймеру для NAT keep-alive).
- **«Занято» (N2) снимается не транзиентностью, а отказом от авто-ответа:** держим
  регистрацию (forking-член наравне с телефоном — телефон продолжает звонить), а
  `200 OK` шлём **только по явному действию пользователя** в окне `CallInvalidated`
  (~30с). Авто-ответ «забирает» вызов (наблюдалось: телефон получил
  `CALL_END_ANSWERED_MOBILE` при авто-ответе пробы) — поэтому он под запретом (PRD F1).
- ⚠️ Под-вопрос на этап интеграции: делает ли held-регистрация **без ответа** линию
  «Занято» сама по себе (вероятно нет — это ещё одно устройство в forking), проверить
  при сборке сетевого слоя.
