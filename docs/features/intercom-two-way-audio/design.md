...
### 3.1. SIP-стек: `voip-utils` (HA core) + наш G.711/REGISTER/STUN-слой
...
`probe_sip_media.py` доказал, что SIP-UAS реализуем на **чистом `asyncio` + `audioop`
+ `socket`**, без тяжёлых зависимостей (никакого aiortc/PJSIP). Но ручной фреймер
хрупок (PRD N3). Research 2026-06-22 (см. §3.5) нашёл лучшую базу — **`voip-utils`**
(Apache-2.0, **SIP-клиентский слой из самого HA core**, на нём работает встроенная
VoIP-интеграция): чистый Python, py3.13-clean, **уже UAS** (принимает прямой INVITE,
отвечает 200 OK, **уже эхо-ит `Via`/`Record-Route`/`From`/`To`/`Call-ID`** — критично
для Kazoo).

**План:** SIP-сигнализацию строим на `voip-utils`, дописываем недостающее из нашего
доказанного `probe`:
- **G.711 PCMU/PCMA** (в `voip-utils` кодек захардкожен Opus) — через `audioop`.
- **REGISTER + Digest MD5** к realm оператора (`voip-utils` рассчитан на direct-INVITE без REGISTER).
- **STUN + symmetric RTP-latching** (в `voip-utils` нет — он LAN-ориентирован).

Это закрывает N3 «настоящий SIP-стек, не ручной фреймер» **без тяжёлых зависимостей**
и работает в **HA Container**. Fallback (если публичный API `voip-utils` не даст
переопределить кодек/SDP) — ручной `asyncio`-модуль `sip.py` из `probe`.
...
|---|---|---|
| **`voip-utils`** (HA core) | Apache-2.0 | **SIP-сигнализация как база** (§3.1): UAS, эхо заголовков, py3.13. Допиливаем G.711/REGISTER/STUN |
...
1. **Transient-регистрация и тайминг.** Успеть зарегистрироваться к моменту форк-INVITE,
   не держать постоянно (side-effect «Занято»). Триггер — FCM `CALL_INCOMING` (~0.5–1с),
   INVITE форкается следом. Нужен probe-прогон: успевает ли transient-register до INVITE,
   или держать короткое окно регистрации по сигналу. **Риск: тайминг.**
