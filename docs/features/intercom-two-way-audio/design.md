...
1. ✅ **Регистрация — HELD (D2 решён, спайк 2026-06-23).** Замер вживую: SIP `INVITE`
   опережает FCM `CALL_INCOMING` на **~107 мс** → transient-register по FCM **невозможна**
   (нет окна). Держим регистрацию **постоянно** (re-REGISTER keep-alive, как `probe`).
   «Занято» (N2) снимается **отказом от авто-ответа**: `200 OK` только по явному действию
   пользователя в окне `CallInvalidated`. См. [research-spike.md](research-spike.md) D2.
