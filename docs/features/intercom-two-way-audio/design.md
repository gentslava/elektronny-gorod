...
1. ✅ **Регистрация — HELD (D2 решён 2 live-тестами, спайк 2026-06-23).**
   - Тест push-to-register: по FCM `INVITE` **не приходит** (forking проходит без поздней
     регистрации) → push-wake без предв. binding не работает.
   - Тест held без авто-ответа: `INVITE` **приходит**, **телефон принимает штатно, панель
     не «занято»** → held не вредит.
   → Держим регистрацию **постоянно** (re-REGISTER keep-alive). На `INVITE` **не
   авто-отвечаем** (`180 Ringing`); `200 OK` — только по явному «ответить» в окне
   `CallInvalidated`. «Занято» = артефакт авто-ответа, не регистрации.
   См. [research-spike.md](research-spike.md) D2.
