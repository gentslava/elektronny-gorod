...
| Slice | Что | Проверяемый результат | Статус |
|---|---|---|---|
| **0** | SIP-пакет `sip/` (13 модулей). Юнит-тесты: парсинг SDP, Digest MD5, multi-Via, BYE-диалог. | Тесты зелёные + ручной приём вызова с логом RTP | ✅ DONE |
| **1** | FCM `CALL_INCOMING` → register-on-ring (ADR-0012): mint → hold INVITE → по «Ответить» `200 OK` → RTP-latching. Downlink: `bridge.py` → go2rtc → `camera.intercom_call`. | **Слышим гостя** + показ экрана вызова | ✅ DONE |
| **2** | **uplink**: go2rtc WebRTC-backchannel → `AudioBridge` обратная ветка → `SipManager.uplink_provider` → RTP в домофон. PoC до code. | **Полный разговор** | ⬜ TODO |
| **3** | Polish: DTMF для открытия двери, edge cases, тесты uplink. | Готово к релизу | ⬜ TODO |
| позже | go2rtc SIP-source на Go (upstream-PR) | вклад в сообщество | — |

## 6. Технические развилки / открытые вопросы (добить в плане/экспериментом)

1. ✅ **Регистрация — register-on-ring / held-short-window ([ADR-0012](../../decisions/0012-register-on-ring.md), 2026-06-23).**
   ...
   Эволюция: изначально предполагалась register-on-answer (pcap 2026-06-22); затем доказано
   (pcap `captures/panel_drop.pcap`), что приложение держит регистрацию во время ring и
   получает `CANCEL` → ADR-0012 зафиксировал register-on-ring как итоговую стратегию.
...
- [x] При FCM `CALL_INCOMING` — `sip.py` регистрируется и держит INVITE (`100 Trying`); при явном `answer` — `200 OK` на held-INVITE, поднимается RTP.
- [x] Сброс с панели → SIP `CANCEL` → мгновенный dismiss экрана вызова в HA (ADR-0012).
- [x] Пользователь **слышит** гостя в HA — `AudioBridge` → go2rtc → HA-native WebRTC (`camera.intercom_call`).
- [ ] Пользователь **говорит** гостю через микрофон браузера (Slice 2 — uplink).
...
- [x] Регистрация не блокирует линию («Занято» — артефакт авто-ответа, мы шлём `100 Trying`).
- [ ] Открытие двери в разговоре через DTMF / уже работает REST.

## Quality gate

`SPEC_READY` ✅ | `IMPL_DONE` (Slice 0+1) ✅ | `SLICE_2_PENDING` (uplink)
