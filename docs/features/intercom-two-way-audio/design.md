...
> Актуальный source of truth для сигнализации —
> [call-answer-model.md](call-answer-model.md). Ранние гипотезы ниже сохраняются
> только как история решений и не отменяют повторную PCAP-верификацию 2026-07-13.
...
|---|---|---|---|
| **0** | SIP-пакет `sip/` (14 модулей, включая `uplink.py`). Юнит-тесты: парсинг SDP, Digest MD5, multi-Via, BYE-диалог. | Тесты зелёные + ручной приём вызова с логом RTP | ✅ DONE |
...
| **2** | **uplink**: браузер → HA WebSocket binary-audio → `UplinkSink` → `SipManager.uplink_provider` → RTP в домофон (ADR-0013). | **Полный разговор** | ✅ DONE |
| **3** | Polish: edge cases, карточка вызова, multi-call, video anti-churn; DTMF остаётся вне текущего scope. | Готово к релизной проверке | 🟡 PARTIAL |
...
- [x] Пользователь **говорит** гостю через микрофон браузера (Slice 2 / ADR-0013).
