...
По клику интеграция запрашивает подписанную ссылку
`forpost/events/{id}/downloads` и отдаёт её плееру. Оператор готовит mp4
по требованию: первый клик может занять до ~30 секунд (сервер рендерит
клип, интеграция опрашивает статус). Ссылка нигде не сохраняется и не
логируется; каждый запуск получает свежую ссылку. Ошибка «Archive is
