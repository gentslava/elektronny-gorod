...
- Для HA lifecycle / Repairs / config flow / coordinator / entity / manifest дополнительно обязателен `ha-expert`. Для auth, токенов, credentials, headers, логирования, crypto, diagnostics и FCM — `security-auditor`. Для изменений tests/fixtures/test plan — `qa-engineer`. Финальные profile reviews read-only. Read-only распространяется и на пробы: мутации, временные тесты и любые правки для проверки гипотезы выполняются только в изолированном worktree, никогда в рабочей копии. Правка дерева под чужим прогоном даёт логически невозможные падения — pytest берёт строку исходника в момент отчёта, а объяснение ассерта из выполненного байткода, — и такой прогон читается как флейк в тестах.
- Critical и Important findings разбираются и закрываются до merge. Находка — это утверждение о дефекте, а не наряд на работу: implementer сначала воспроизводит дефект, потом решает, как его закрыть. Предложенная ревьюером правка — рекомендация; принять её дословно, ухудшив код, хуже, чем возразить. Обоснованный отказ — законный исход: фиксируется как `WON'T FIX` с аргументом и остаётся под тем же gate. Правка по находке не должна порождать новый дефект — недостижимый код, лишнюю сущность или проверку, дублирующую тип.
- Если независимый reviewer недоступен, gate остаётся незакрытым; self-review не подменяет evidence.
