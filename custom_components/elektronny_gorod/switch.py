...
# исходящие вызовы действий. Слать оператору пачку переключений одного адреса
# незачем — сериализуем. От потери переключений это само по себе не спасает:
# их разводит запись принятого payload в снимок координатора
# (`async_set_dnd`), потому что дело в устаревших данных, а не в порядке.
PARALLEL_UPDATES = 1
