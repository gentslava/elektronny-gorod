...
def _build_go2rtc_src(source_url: str) -> str:
    # Рабочий рецепт: видео copy, аудио -> AAC/OPUS (go2rtc дальше отдаст в WebRTC/HLS как надо)
...
class ElektronnyGorodCamera(Camera):
    ...
    async def _ensure_go2rtc_stream(self) -> None:
        ...
        source_url = await self._coordinator.get_camera_stream(self._id)
        if not source_url:
    ...
    async def stream_source(self) -> str | None:
        if not self._use_go2rtc:
            url = await self._coordinator.get_camera_stream(self._id)
            if not url:
                LOGGER.warning("Camera %s (%s): empty source stream url", self._name, self._id)
            return url
