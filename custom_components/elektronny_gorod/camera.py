...
# семафор платформы и дёргает оператора на каждую сущность. Ограничивать его
# нечем — превью в интерфейсе ходит мимо семафора (`CameraImageView`), так что
# сериализация действий защитила бы только часть обращений, зато замедлила бы
# панель с десятком камер. Наплыв к оператору держат кэш снимка и
# `_snapshot_retry_after`, а не эта константа.
PARALLEL_UPDATES = 0
